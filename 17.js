...
highly detailed, masterpiece, best quality
                    ...
                    if (result) {
                        ...
                        if (result.startsWith('data:image/')) {
                            try {
                                ...
                                const savedUrl = await saveBase64AsFile(result, charName, fileName, ext);
                    ...
                    <div style="margin-top: 20px; padding: 15px; background: linear-gradient(145deg, #252530, #1e1e24); border-radius: 8px; box-shadow: 3px 3px 6px var(--nm-shadow-dark), -2px -2px 5px var(--nm-shadow-light);">
                        ...
                        <div class="sd-api-row">
                            ...
                            <select id="sd-novelai-resolution" class="text_pole">
                                <option value="640x480" ${settings.novelaiParams.width === 640 && settings.novelaiParams.height === 480 ? 'selected' : ''}>640x480</option>
                                <option value="832x1216" ${settings.novelaiParams.width === 832 && settings.novelaiParams.height === 1216 ? 'selected' : ''}>832x1216 (竖屏)</option>
                                <option value="1216x832" ${settings.novelaiParams.width === 1216 && settings.novelaiParams.height === 832 ? 'selected' : ''}>1216x832 (横屏)</option>
                                <option value="1024x1024" ${settings.novelaiParams.width === 1024 && settings.novelaiParams.height === 1024 ? 'selected' : ''}>1024x1024 (方形)</option>
                                <option value="512x512" ${settings.novelaiParams.width === 512 && settings.novelaiParams.height === 512 ? 'selected' : ''}>512x512 (小)</option>
                                <option value="1920x1080" ${settings.novelaiParams.width === 1920 && settings.novelaiParams.height === 1080 ? 'selected' : ''}>1920x1080 (1080P)</option>
                            </select>
                        </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-test-novelai').on('click', async function () {
                ...
                try {
                    ...
                    settings.novelaiPrefix = originalPrefix;
                    settings.novelaiNegative = originalNegative;
                    toastr.error(`❌ NovelAI 测试失败: ${e.message}`);
                } finally {
    ...
    /**
     * 将 base64 图片保存到本地
     * @param {string} base64Data - base64 图片数据 (格式: data:image/png;base64,...)
     * @param {string} charName - 角色名称
     * @param {string} fileName - 文件名
     * @param {string} ext - 文件扩展名
     * @returns {Promise<string>} - 返回保存后的文件路径
     */
    async function saveBase64AsFile(base64Data, charName, fileName, ext) {
        try {
            // 检查是否存在 SillyTavern 的 saveBase64AsFile 函数
            if (typeof window !== 'undefined' && typeof window.SillyTavern !== 'undefined' && typeof window.SillyTavern.saveBase64AsFile === 'function') {
                return await window.SillyTavern.saveBase64AsFile(base64Data, charName, fileName, ext);
            } else {
                // 如果没有找到酒馆的保存函数，则模拟保存行为，返回一个虚拟路径
                // 在实际环境中，这应该调用酒馆的图片保存功能
                const timestamp = Date.now();
                return `/output/${charName || 'unknown'}/${fileName}_${timestamp}.${ext}`;
            }
        } catch (error) {
