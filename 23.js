...
highly detailed, masterpiece, best quality
                        ...
                        if (result.startsWith('data:image/')) {
                            ...
                            try {
                                ...
                                if (typeof window !== 'undefined' && typeof window.SillyTavern !== 'undefined' && typeof window.SillyTavern.saveBase64AsFile === 'function') {
                                    ...
                                    // 严格禁止base64模式的图片储存，所以不将base64数据保存到聊天文本中
                                    // 只在UI中显示，但不保存到聊天记录
                                    uniqueImages = [...new Set([...state.images])]; // 不添加base64图片到文本存储的列表
                                    // 直接更新 UI，不更新聊天文本
                                    setTimeout(() => {
                                        const $newWrap = $(`.mes[mesid="${state.mesId}"] .sd-ui-wrap[data-block-idx="${state.blockIdx}"]`);
                                        if ($newWrap.length) {
                                            // 在这里临时显示base64图片，但不保存到文本
                                            $newWrap.attr('data-images', encodeURIComponent(JSON.stringify([...state.images, result])));
                                            updateWrapperView($newWrap, [...state.images, result], [...state.images, result].length - 1);
                                        }
                                    }, 200);
                                }
                            } catch (saveError) {
            ...
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div id="sd-tab-novelai" class="sd-tab-content">
                    ...
                    <div style="margin-top: 15px;">
                        <label style="display:block; margin-bottom:5px;">测试提示词</label>
                        <input type="text" id="sd-novelai-test-prompt" class="text_pole" placeholder="输入测试提示词，例如：1girl" value="1girl">
                    </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-test-novelai').on('click', async function () {
                ...
                try {
                    ...
                    // 使用用户输入的测试提示词，使用当前设置的所有参数
                    const result = await callNovelAIForImage(testPrompt, {
                        width: width || 1216,
                        height: height || 832,
                        scale: parseFloat($('#sd-novelai-scale').val()) || 6,
                        ...
                        decrisper: $('#sd-novelai-decrisper').is(':checked'),
                        variety_boost: $('#sd-novelai-variety-boost').is(':checked')
                    }, $('#sd-novelai-prefix').val(), $('#sd-novelai-negative').val()); // 传递正面和负面提示词
    ...
    async function callNovelAIForImage(prompt, params = {}, prefix, negative) {
        ...
        try {
            ...
            const filename = `${charName}_${timestamp}.png`;
            const file = new File([blob], filename, { type: 'image/png' });

            // 使用酒馆的图片上传功能
            if (typeof window.SillyTavern !== 'undefined' && typeof window.SillyTavern.uploadFile === 'function') {
                try {
                    const uploadResult = await window.SillyTavern.uploadFile(file, 'images');
                    if (uploadResult && uploadResult.url) {
                        addLog('NOVELAI', `NovelAI 图片已保存到本地: ${uploadResult.url}`);
                        return uploadResult.url; // 返回图片的URL
                    }
                } catch (uploadError) {
                    console.error('上传图片到酒馆失败:', uploadError);
                }
            }

            // 如果上传失败，尝试使用其他方式上传
            // 注意：我们不直接使用fetch上传到不确定的端点，而是依赖酒馆的API
            // 如果SillyTavern.uploadFile不可用，我们可以尝试其他酒馆提供的上传方法
            if (typeof window.SillyTavern !== 'undefined' && typeof window.SillyTavern.saveBase64AsFile === 'function') {
                try {
                    // 使用SillyTavern的saveBase64AsFile方法
                    const imageUrl = await window.SillyTavern.saveBase64AsFile(`data:image/png;base64,${base64}`, charName, `novelai_${Date.now()}`, 'png');
                    if (imageUrl) {
                        addLog('NOVELAI', `NovelAI 图片已保存到本地: ${imageUrl}`);
                        return imageUrl;
                    }
                } catch (saveAsFileError) {
                    console.error('使用saveBase64AsFile保存图片失败:', saveAsFileError);
                }
            }

            // 如果所有保存方式都失败，返回base64作为备选方案
            addLog('NOVELAI', `NovelAI 图片生成成功但保存到本地失败，使用base64格式`);
