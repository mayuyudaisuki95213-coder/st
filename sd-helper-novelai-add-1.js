...
highly detailed, masterpiece, best quality
                ...
                try {
                    ...
                    if (result) {
                        ...
                        if (result.startsWith('data:image/')) {
                            // 使用酒馆的图片保存机制处理 base64 图片
                            try {
                                // 尝试通过酒馆的接口保存图片
                                const charName = getCurrentCharacterName() || 'unknown';
                                const fileName = `novelai_${Date.now()}_${state.blockIdx}`;
                                // 从 base64 数据中提取扩展名
                                const ext = result.startsWith('data:image/png') ? 'png' :
                                          result.startsWith('data:image/jpeg') ? 'jpg' : 'png';

                                // 检查是否可以通过酒馆的接口保存
                                if (typeof window !== 'undefined' && typeof window.SillyTavern !== 'undefined' && typeof window.SillyTavern.saveBase64AsFile === 'function') {
                                    const savedUrl = await window.SillyTavern.saveBase64AsFile(result, charName, fileName, ext);

                                    // 使用保存后的 URL 更新
                                    uniqueImages = [...new Set([...state.images, savedUrl])];
                                    ...
                                    // 如果无法通过酒馆保存，暂时使用 base64 数据但不存储到文本
                                    // 为了兼容性，我们仍然尝试保存，但会将图片保存到文本中
                                ...
                                console.error('保存 NovelAI 图片失败:', saveError);
                                // 如果保存失败，仍然使用 base64 数据，但不存储到文本
                                uniqueImages = [...new Set([...state.images, result])];
                                // 直接更新 UI，不更新聊天文本
                                setTimeout(() => {
                                    const $newWrap = $(`.mes[mesid="${state.mesId}"] .sd-ui-wrap[data-block-idx="${state.blockIdx}"]`);
    ...
    function updateWrapperView($wrap, images, idx) {
        ...
        if (count === 0) {
            ...
            if (images[idx].startsWith('data:image/')) {
    ...
    async function streamingGenerateImage(prompt) {
        ...
        try {
            if (settings.novelaiEnabled) {
                ...
                // 如果是 base64 图片，返回 base64 数据；否则返回 URL
                return result;
            } else {
                // 使用酒馆自带生图
