...
highly detailed, masterpiece, best quality
                    ...
                    if (result) {
                        ...
                        if (result.startsWith('data:image/')) {
                            ...
                            try {
                                ...
                                const fileName = `${charName}_novelai_${timestamp}`;
                                // 从 base64 数据中提取扩展名
                                ...
                                if (typeof window !== 'undefined' && typeof window.SillyTavern !== 'undefined' && typeof window.SillyTavern.saveBase64AsFile === 'function') {
                                    ...
                                    // 如果没有可用的酒馆API方法，我们不保存base64到文本，而是显示错误信息
                                    console.error('SillyTavern.saveBase64AsFile方法不可用，无法保存图片到本地');
                                    // 不将base64保存到文本，而是使用空的图片数组
                                    uniqueImages = [...new Set([...state.images])];
                                    await updateChatData(state.mesId, state.blockIdx, state.prompt, uniqueImages, false, false);
                                    setTimeout(() => {
                                        const $newWrap = $(`.mes[mesid="${state.mesId}"] .sd-ui-wrap[data-block-idx="${state.blockIdx}"]`);
                                        if ($newWrap.length) updateWrapperView($newWrap, uniqueImages, uniqueImages.length - 1);
    ...
    async function callNovelAIForImage(prompt, params = {}, prefix, negative) {
        ...
        try {
            ...
            // 使用SillyTavern的saveBase64AsFile方法将图片保存到本地酒馆目录
            const charName = getCurrentCharacterName() || 'unknown';
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-').replace('T', '@').replace('Z', '');
            const filename = `${charName}_novelai_${timestamp}`;

            // 直接使用SillyTavern的saveBase64AsFile方法，这是将图片保存到本地的标准方法
            if (typeof window !== 'undefined' && typeof window.SillyTavern !== 'undefined' && typeof window.SillyTavern.saveBase64AsFile === 'function') {
                try {
                    const imageUrl = await window.SillyTavern.saveBase64AsFile(`data:image/png;base64,${base64}`, charName, filename, 'png');
                    if (imageUrl) {
                        addLog('NOVELAI', `NovelAI 图片已保存到本地: ${imageUrl}`);
                        return imageUrl; // 返回保存后的URL
                    }
                } catch (saveAsFileError) {
                    console.error('使用SillyTavern.saveBase64AsFile保存图片失败:', saveAsFileError);
                }
            }

            // 如果SillyTavern的API不可用，尝试其他方法
            // 将base64图片数据转换为blob并尝试使用其他方式
