    ...
    function gmFetch(url, options = {}) {
        return new Promise((resolve, reject) => {
            GM_xmlhttpRequest({
                ...
                responseType: 'arraybuffer', // 添加这一行以支持二进制数据
                timeout: 60000,  // 60秒超时
                onload: (response) => {
                    const res = {
...
highly detailed, masterpiece, best quality
    ...
    const DEFAULT_SETTINGS = {
        ...
        novelaiModel: 'nai-diffusion-3',   // NovelAI 模型名称
        novelaiParameters: {               // NovelAI 详细参数
            sampler: 'k_euler',
            steps: 28,
            scale: 5,
            noise: 0.2,
    ...
    async function handleGeneration(state) {
        ...
        if (settings.novelaiEnabled && settings.novelaiApiKey) {
            ...
            state.el.msg.text('⏳ NovelAI请求中...').addClass('show');
            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    if (attempt > 1) {
                        state.el.msg.text(`⏳ NovelAI重试中 (${attempt}/${MAX_RETRIES})...`);
                        addLog('GENERATION', `第${attempt}次重试 NovelAI 生图...`);
                    }

                    // 调用 NovelAI API
                    const imageUrl = await callNovelAIApi(finalPrompt, settings.novelaiParameters);

                    if (imageUrl) {
                        state.el.msg.text('✅ NovelAI成功');
                        const uniqueImages = [...new Set([...state.images, imageUrl])];
            ...
            if (lastError) {
                if (lastError.message === '无结果') {
                    state.el.msg.text('⚠️ NovelAI无结果');
                } else {
                    state.el.msg.text(lastError.message.includes('超时') ? '⏱️ NovelAI超时' : '❌ NovelAI错误');
                }
                addLog('GENERATION', `NovelAI 生图失败（已重试${MAX_RETRIES}次）: ${lastError.message}`);
            }
        } else {
            // 使用原来的酒馆生图命令
            const finalPrompt = `${settings.globalPrefix ? settings.globalPrefix + ', ' : ''}${state.prompt}${settings.globalSuffix ? ', ' + settings.globalSuffix : ''}`.replace(/,\s*,/g, ',').trim();
    ...
    function updateWrapperView($wrap, images, idx) {
        ...
        if (count === 0) {
            ...
            $ph.hide();
            // 对于 base64 图片，不需要额外编码
            const imageUrl = images[idx].startsWith('data:image/') ? images[idx] : encodeImageUrl(images[idx]);
    ...
    function parseBlockContent(raw) {
        ...
        const base64Images = (text.match(base64Regex) || []).map(url => url.trim());

        // 合并两种类型的图片URL
        const images = [...urlImages, ...base64Images];

        let prompt = text
            .replace(urlRegex, '')      // 移除普通URL
            .replace(base64Regex, '')   // 移除base64图片数据
            .replace(NO_GEN_FLAG, '')
    ...
    function createUIHtml(prompt, images, prevent, blockIdx, initIdx, isScheduled = false) {
        ...
        // 对于 base64 图片，不需要额外编码
        const imageUrl = has && images[initIdx] ? (images[initIdx].startsWith('data:image/') ? images[initIdx] : encodeImageUrl(images[initIdx])) : '';
        return `
        <div class="sd-ui-container">
            <div class="sd-ui-wrap" data-prompt="${encodeURIComponent(prompt)}" data-images="${encodeURIComponent(JSON.stringify(images))}" data-prevent-auto="${prevent}" data-block-idx="${blockIdx}" data-cur-idx="${initIdx}" data-scheduled="${isScheduled}">
                ...
                <div class="sd-ui-viewport">
                    ...
                    <img class="sd-ui-image" src="${imageUrl}" style="display:${has ? 'block' : 'none'}" />
    ...
    // NovelAI API 生图函数
    async function callNovelAIApi(prompt, params) {
        if (!settings.novelaiEnabled || !settings.novelaiApiKey) {
            throw new Error("NovelAI API 未启用或未配置 API Key");
        }
        ...
        const requestParams = {
            ...
            parameters: {
                prompt: prompt,
                negative_prompt: params.negative_prompt || settings.globalNegative || 'lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry',
                seed: params.seed || -1,
                qualityToggle: params.qualityToggle !== undefined ? params.qualityToggle : true,
                cfg_rescale: params.cfg_rescale || 0,
                noise_schedule: "native",
                legacy: false,
                add_original_image: params.addOriginalImage !== undefined ? params.addOriginalImage : true,
                sampler: params.sampler || "k_euler",
                steps: params.steps || 28,
                scale: params.scale || 5,
                sm: params.sm !== undefined ? params.sm : true,
                smea: params.smea !== undefined ? params.smea : true,
                dynamic_thresholding: params.dynamicThresholding !== undefined ? params.dynamicThresholding : false,
                controlnet_strength: 1,
                image_strength: params.strength || 0.7,
                noise: params.noise || 0.2,
                ...
                highres_fix: params.highresFix !== undefined ? params.highresFix : false,
                hires_steps: params.highresFix ? (params.hiresFixSteps || 15) : 0,
                hires_scale: params.highresFix ? (params.hiresFixScale || 1.5) : 1,
        ...
        if (params.highresFix) {
            requestParams.parameters.hires_steps = params.hiresFixSteps || 15;
            requestParams.parameters.hires_scale = params.hiresFixScale || 1.5;
        }

        try {
            addLog('NOVELAI', `请求 NovelAI API: ${url}`);
            addLog('NOVELAI', `模型: ${requestParams.model}, 提示词: ${prompt.substring(0, 100)}...`);
            ...
            // NovelAI 返回的是二进制数据，需要转换为 base64
            const arrayBuffer = await response.arrayBuffer();
            const base64 = arrayBufferToBase64(arrayBuffer);
            const imageUrl = `data:image/png;base64,${base64}`;

            addLog('NOVELAI', `图片生成成功，大小: ${base64.length} 字符`);
            return imageUrl;
        } catch (error) {
    ...
    function openSettingsPopup() {
        ...
        const html = `
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div class="sd-tab-nav">
                    ...
                    <div class="sd-tab-btn" data-tab="novelai-api">NovelAI API</div>
                ...
                <div id="sd-tab-novelai-api" class="sd-tab-content">
                    <div style="margin-bottom: 15px; padding: 12px; background: linear-gradient(145deg, #252530, #1e1e24); border-radius: 8px; box-shadow: 3px 3px 6px var(--nm-shadow-dark), -2px -2px 5px var(--nm-shadow-light);">
                        <label style="display:block; margin-bottom:8px; font-weight:600;">🔌 NovelAI API 开关</label>
                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                            <input type="checkbox" id="sd-novelai-enabled" ${settings.novelaiEnabled ? 'checked' : ''}>
                            <span style="font-weight: bold;">启用 NovelAI API 直连</span>
                        </label>
                        <small style="color: #888; display: block; margin-top: 6px;">
                            开启后将使用 NovelAI API 直接生图，绕过酒馆内置生图功能
                        </small>
                    </div>
                    ...
                    <div class="sd-api-row">
                        <label>模型名称</label>
                        <input type="text" id="sd-novelai-model" class="text_pole" placeholder="例如: nai-diffusion-3" value="${settings.novelaiModel}">
                    </div>

                    <h4 style="margin-top:20px; margin-bottom:15px;">详细参数设置</h4>

                    <div class="sd-api-row">
                        <label>采样器</label>
                        <select id="sd-novelai-sampler" class="text_pole">
                            <option value="k_euler" ${settings.novelaiParameters.sampler === 'k_euler' ? 'selected' : ''}>Euler</option>
                            <option value="k_euler_ancestral" ${settings.novelaiParameters.sampler === 'k_euler_ancestral' ? 'selected' : ''}>Euler A</option>
                            <option value="k_dpmpp_2m" ${settings.novelaiParameters.sampler === 'k_dpmpp_2m' ? 'selected' : ''}>DPM++ 2M</option>
                            <option value="k_dpmpp_2s_ancestral" ${settings.novelaiParameters.sampler === 'k_dpmpp_2s_ancestral' ? 'selected' : ''}>DPM++ 2S A</option>
                            <option value="k_dpmpp_sde" ${settings.novelaiParameters.sampler === 'k_dpmpp_sde' ? 'selected' : ''}>DPM++ SDE</option>
                            <option value="k_lms" ${settings.novelaiParameters.sampler === 'k_lms' ? 'selected' : ''}>LMS</option>
                        </select>
                    </div>

                    <div class="sd-api-row">
                        <label>步数</label>
                        <input type="number" id="sd-novelai-steps" class="text_pole" value="${settings.novelaiParameters.steps}" min="1" max="50">
                    </div>
                    ...
                    <div class="sd-api-row">
                        <label>种子</label>
                        <input type="number" id="sd-novelai-seed" class="text_pole" value="${settings.novelaiParameters.seed}" min="-1" max="999999999">
                    </div>

                    <div class="sd-api-row">
                        <label>负面提示词</label>
                        <textarea id="sd-novelai-negative" class="text_pole" rows="3" style="width:100%">${settings.novelaiParameters.negative_prompt}</textarea>
                    </div>

                    <div class="sd-api-row" style="display: flex; align-items: center;">
                        <label style="flex: 0 0 auto; margin-right: 10px;">高分辨率修复</label>
                        <input type="checkbox" id="sd-novelai-highres-fix" ${settings.novelaiParameters.highresFix ? 'checked' : ''} style="margin-right: 5px;">
                        <span>启用</span>
                    </div>

                    <div class="sd-api-row">
                        <label>高分修复比例</label>
                        <input type="number" id="sd-novelai-hires-scale" class="text_pole" value="${settings.novelaiParameters.hiresFixScale}" min="1" max="4" step="0.1">
                    </div>

                    <div class="sd-api-row">
                        <label>高分修复步数</label>
                        <input type="number" id="sd-novelai-hires-steps" class="text_pole" value="${settings.novelaiParameters.hiresFixSteps}" min="1" max="50">
                    </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-save').on('click', () => {
                ...
                // NovelAI API 设置
                settings.novelaiEnabled = $('#sd-novelai-enabled').is(':checked');
                settings.novelaiApiKey = $('#sd-novelai-key').val();
                settings.novelaiModel = $('#sd-novelai-model').val();
                settings.novelaiParameters.sampler = $('#sd-novelai-sampler').val();
                settings.novelaiParameters.steps = parseInt($('#sd-novelai-steps').val()) || 28;
                settings.novelaiParameters.scale = parseFloat($('#sd-novelai-scale').val()) || 5;
                settings.novelaiParameters.noise = parseFloat($('#sd-novelai-noise').val()) || 0.2;
                settings.novelaiParameters.strength = parseFloat($('#sd-novelai-strength').val()) || 0.7;
                settings.novelaiParameters.seed = parseInt($('#sd-novelai-seed').val()) || -1;
                settings.novelaiParameters.negative_prompt = $('#sd-novelai-negative').val();
                settings.novelaiParameters.highresFix = $('#sd-novelai-highres-fix').is(':checked');
                settings.novelaiParameters.hiresFixScale = parseFloat($('#sd-novelai-hires-scale').val()) || 1.5;
                settings.novelaiParameters.hiresFixSteps = parseInt($('#sd-novelai-hires-steps').val()) || 15;
            ...
            $('#sd-test-novelai-api').on('click', async function () {
                ...
                try {
                    // 临时保存当前设置以便测试
                    const oldSettings = { ...settings.novelaiParameters };
                    const testParams = {
                        ...
                        negative_prompt: $('#sd-novelai-negative').val(),
                        highresFix: $('#sd-novelai-highres-fix').is(':checked'),
                        hiresFixScale: parseFloat($('#sd-novelai-hires-scale').val()) || 1.5,
                        hiresFixSteps: parseInt($('#sd-novelai-hires-steps').val()) || 15
                    };

                    // 临时更新设置
                    settings.novelaiApiKey = apiKey;
                    settings.novelaiModel = model;
                    settings.novelaiParameters = { ...settings.novelaiParameters, ...testParams };

                    // 测试生成一张简单的图片
                    const result = await callNovelAIApi('1girl, solo, blue hair, red eyes', settings.novelaiParameters);

                    if (result) {
                        toastr.success('✅ NovelAI API 测试成功！');
                    } else {
                        toastr.error('❌ NovelAI API 测试失败');
                    }

                    // 恢复原始设置
                    settings.novelaiParameters = oldSettings;
                } catch (e) {
                    toastr.error(`❌ NovelAI API 测试失败: ${e.message}`);
                } finally {
