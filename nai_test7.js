...
highly detailed, masterpiece, best quality
    ...
    const DEFAULT_SETTINGS = {
        ...
        novelaiModel: 'nai-diffusion-3',    // NovelAI 模型名
        novelaiParameters: {                // NovelAI 参数配置
            seed: -1,                       // 随机种子
            steps: 28,                      // 采样步数
            cfg_rescale: 0,                 // CFG缩放
            noise_schedule: 0.3,            // 噪声调度
            strength: 0.7,                  // 强度
            sampler: 'k_euler_ancestral',   // 采样器
            scale: 11,                      // 提示词引导系数
            negative_prompt: '',            // 负面提示词
            quality_tags: true,             // 质量标签
            nsfw: true,                     // NSFW内容
            dynamic_thresholding: false,    // 动态阈值
            sm: false,                      // sm参数
            sm_dyn: false,                  // sm_dyn参数
            ucPreset: 0,                   // ucPreset参数
            reference_image_multiple: null,  // 参考图像
            reference_information_extracted_multiple: null, // 参考信息提取
            reference_strength_multiple: null, // 参考强度
            ar: 1.78,                      // 宽高比
            height: 1024,                  // 高度
            width: 1792,                   // 宽度
            image: null                    // 参考图像base64
        },
    ...
    async function callNovelAIApi(prompt) {
        ...
        const parameters = {
            ...
            prompt: prompt,
            negative_prompt: settings.novelaiParameters.negative_prompt || settings.globalNegative
        };

        // 确保必要的参数存在
        if (parameters.width === undefined) parameters.width = settings.novelaiParameters.width || 1792;
        if (parameters.height === undefined) parameters.height = settings.novelaiParameters.height || 1024;
        if (parameters.steps === undefined) parameters.steps = settings.novelaiParameters.steps || 28;
        if (parameters.scale === undefined) parameters.scale = settings.novelaiParameters.scale || 11;
        if (!parameters.sampler) parameters.sampler = settings.novelaiParameters.sampler || 'k_euler_ancestral';
        ...
        const requestBody = {
            ...
            parameters: {
                ...
                seed: parameters.seed !== undefined ? parameters.seed : -1,
                // 提示词相关
                prompt: prompt,
                negative_prompt: parameters.negative_prompt || settings.globalNegative,
                // 可选参数
                ...(parameters.cfg_rescale !== undefined ? { cfg_rescale: parameters.cfg_rescale } : {}),
                ...(parameters.noise_schedule !== undefined ? { noise_schedule: parameters.noise_schedule } : {}),
                ...(parameters.strength !== undefined ? { strength: parameters.strength } : {}),
                ...(parameters.quality_tags !== undefined ? { quality_tags: parameters.quality_tags } : {}),
                ...(parameters.nsfw !== undefined ? { nsfw: parameters.nsfw } : {}),
                ...(parameters.dynamic_thresholding !== undefined ? { dynamic_thresholding: parameters.dynamic_thresholding } : {}),
                ...(parameters.sm !== undefined ? { sm: parameters.sm } : {}),
                ...(parameters.sm_dyn !== undefined ? { sm_dyn: parameters.sm_dyn } : {}),
                ...(parameters.ucPreset !== undefined ? { ucPreset: parameters.ucPreset } : {}),
                ...(parameters.reference_image_multiple ? { reference_image_multiple: parameters.reference_image_multiple } : {}),
                ...(parameters.reference_information_extracted_multiple ? { reference_information_extracted_multiple: parameters.reference_information_extracted_multiple } : {}),
                ...(parameters.reference_strength_multiple ? { reference_strength_multiple: parameters.reference_strength_multiple } : {}),
            }
        };
        ...
        try {
            ...
            // NovelAI API 返回的是二进制数据，需要转换为blob
            const blob = await response.blob();

            // 将blob转换为base64，以便在前端显示
            return new Promise((resolve, reject) => {
                const reader = new FileReader();
                reader.onload = function() {
                    resolve([reader.result]); // 返回base64 URL数组
                };
    ...
    async function handleGeneration(state) {
        ...
        // 根据设置决定使用哪种生图方式
        const useNovelAI = settings.novelaiEnabled && settings.novelaiApiKey;
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                if (useNovelAI) {
                    ...
                    const result = await callNovelAIApi(finalPrompt);
                    trimmedUrls = result || [];
                } else {
                    // 使用原有生图方式
                    const finalPrompt = `${settings.globalPrefix ? settings.globalPrefix + ', ' : ''}${state.prompt}${settings.globalSuffix ? ', ' + settings.globalSuffix : ''}`.replace(/,\s*,/g, ',').trim();
                    ...
                    // 保持原始URL格式，仅清理尾部空白
                    trimmedUrls = newUrls.map(url => url.trim());
                }
    ...
    function openSettingsPopup() {
            ...
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div style="margin-bottom: 12px;">
                    <div style="display: flex; align-items: center; gap: 8px;">
                        ...
                        <span class="sd-toggle-arrow collapsed" data-target="sd-sub-novelai">▾</span>
                    </div>
                    <small style="color: #888; display: block; margin-left: 24px; margin-top: 4px;">
                        开启后将使用 NovelAI API 直接生成图片
                    </small>
                    <div id="sd-sub-novelai" class="sd-sub-settings collapsed">
                        <div class="sd-api-row">
                        ...
                        <div class="sd-api-row">
                            <label>模型</label>
                            <input type="text" id="sd-novelai-model" class="text_pole" placeholder="模型名称" value="${settings.novelaiModel}">
                        </div>
                        <div class="sd-api-row">
                            <label>宽度</label>
                            <input type="number" id="sd-novelai-width" class="text_pole" value="${settings.novelaiParameters.width}" min="64" max="3072" step="64">
                        </div>
                        <div class="sd-api-row">
                            <label>高度</label>
                            <input type="number" id="sd-novelai-height" class="text_pole" value="${settings.novelaiParameters.height}" min="64" max="3072" step="64">
                        </div>
                        <div class="sd-api-row">
                            <label>采样步数</label>
                        ...
                        <div class="sd-api-row">
                            <label>随机种子</label>
                            <input type="number" id="sd-novelai-seed" class="text_pole" value="${settings.novelaiParameters.seed}" placeholder="-1表示随机">
                        </div>
                        <div class="sd-api-row">
                            <label>负面提示词</label>
                        ...
                        <div class="sd-api-row">
                            <label style="display: flex; align-items: center; gap: 8px;">
                                <input type="checkbox" id="sd-novelai-quality-tags" ${settings.novelaiParameters.quality_tags ? 'checked' : ''}>
                                <span>启用质量标签</span>
                            </label>
                        </div>
                        <div class="sd-api-row">
                            <label style="display: flex; align-items: center; gap: 8px;">
                                <input type="checkbox" id="sd-novelai-nsfw" ${settings.novelaiParameters.nsfw ? 'checked' : ''}>
                                <span>允许NSFW内容</span>
                            </label>
                        </div>
                    </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-save').on('click', () => {
                ...
                settings.novelaiParameters.width = parseInt($('#sd-novelai-width').val()) || 1792;
                settings.novelaiParameters.height = parseInt($('#sd-novelai-height').val()) || 1024;
                settings.novelaiParameters.steps = parseInt($('#sd-novelai-steps').val()) || 28;
                settings.novelaiParameters.scale = parseFloat($('#sd-novelai-scale').val()) || 11;
                ...
                settings.novelaiParameters.quality_tags = $('#sd-novelai-quality-tags').is(':checked');
                settings.novelaiParameters.nsfw = $('#sd-novelai-nsfw').is(':checked');
