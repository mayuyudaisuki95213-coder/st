...
/* ===== NovelAI Extension (Non-intrusive Add-on) ===== */
(function(){
  const NAI_KEY = 'sd_novelai_cfg';
  const defaultCfg = {
    apiKey: '',
  ...
  let cfg = JSON.parse(localStorage.getItem(NAI_KEY) || 'null') || defaultCfg;

  function saveCfg(){ localStorage.setItem(NAI_KEY, JSON.stringify(cfg)); }

  // Simple settings UI injected into page corner
  function injectUI(){
    if(document.getElementById('nai-panel')) return;
    const d = document.createElement('div');
    d.id='nai-panel';
    d.style.cssText='position:fixed;right:10px;bottom:10px;z-index:9999;background:#1e1e24;color:#ddd;padding:10px;border-radius:8px;font-size:12px;width:220px;';
    d.innerHTML = `
      <div style="font-weight:bold;margin-bottom:6px;">NovelAI 生图</div>
      <div>API Key:<input id="nai-key" style="width:100%" /></div>
      <div>Model:<input id="nai-model" style="width:100%" /></div>
      <div>Steps:<input id="nai-steps" type="number" style="width:100%" /></div>
      <div>Scale:<input id="nai-scale" type="number" style="width:100%" /></div>
      <div>W/H:<input id="nai-w" type="number" style="width:45%" /> <input id="nai-h" type="number" style="width:45%" /></div>
      <button id="nai-save" style="width:100%;margin-top:6px;">保存</button>
      <button id="nai-gen" style="width:100%;margin-top:6px;">对当前[IMG_GEN]生图</button>
    `;
    document.body.appendChild(d);
    document.getElementById('nai-key').value = cfg.apiKey;
    document.getElementById('nai-model').value = cfg.model;
    document.getElementById('nai-steps').value = cfg.steps;
    document.getElementById('nai-scale').value = cfg.scale;
    document.getElementById('nai-w').value = cfg.width;
    document.getElementById('nai-h').value = cfg.height;

    document.getElementById('nai-save').onclick = ()=>{
      cfg.apiKey = document.getElementById('nai-key').value.trim();
      cfg.model = document.getElementById('nai-model').value.trim();
      cfg.steps = parseInt(document.getElementById('nai-steps').value)||28;
      cfg.scale = parseFloat(document.getElementById('nai-scale').value)||7;
      cfg.width = parseInt(document.getElementById('nai-w').value)||832;
      cfg.height = parseInt(document.getElementById('nai-h').value)||1216;
      saveCfg();
      alert('已保存 NovelAI 配置');
    };

    document.getElementById('nai-gen').onclick = async ()=>{
      const prompt = getLastImgGenPrompt();
      if(!prompt){ alert('未找到 [IMG_GEN] 提示词'); return; }
      try{
        ...
        alert('NovelAI 生图失败: '+e.message);
      }
    };
  }

  function getLastImgGenPrompt(){
    try{
      if(!window.SillyTavern || !SillyTavern.chat) return null;
      for(let i=SillyTavern.chat.length-1;i>=0;i--){
        const m = SillyTavern.chat[i];
        if(m.mes){
          const match = m.mes.match(/\[IMG_GEN\]([\s\S]*?)\[\/IMG_GEN\]/);
