...
highly detailed, masterpiece, best quality
    ...
    function rebuildBlockString(prompt, images, prevent, scheduled) {
        let res = prompt;
        if (images.length > 0) {
            // 保存所有图片，包括 base64 格式，但要小心处理大型 base64 数据
            // 在某些情况下，我们需要保存 base64 图片以确保它们在会话间持久化
            res += '\n' + images.join('\n');
        }
        else if (prevent) res += '\n' + NO_GEN_FLAG;
    ...
    function openSettingsPopup() {
        ...
        setTimeout(() => {
            ...
            $('#sd-test-novelai').on('click', async function () {
                ...
                const $testPromptInput = $('#sd-novelai-test-prompt');
                const apiKey = $('#sd-novelai-key').val();
                const model = $('#sd-novelai-model').val();
                const testPrompt = $testPromptInput.val() || '1girl';
                ...
                try {
                    ...
                    // 更新设置为当前UI值
                    settings.novelaiApiKey = apiKey;
                    settings.novelaiModel = model;
                    settings.novelaiParams = {
                        width: width || 1216,
                        height: height || 832,
                        scale: parseFloat($('#sd-novelai-scale').val()) || 6,
                    ...
                    const result = await callNovelAIForImage(testPrompt, {
                        ...
                        decrisper: $('#sd-novelai-decrisper').is(':checked'),
                        variety_boost: $('#sd-novelai-variety-boost').is(':checked')
                    }, $('#sd-novelai-prefix').val(), $('#sd-novelai-negative').val());
                    ...
                    // 在测试提示词旁边显示成功状态
                    $testPromptInput.after('<span id="test-status" style="color: green; margin-left: 10px;">测试成功</span>');
                    setTimeout(() => $('#test-status').remove(), 3000);
                } catch (e) {
                    // 恢复原来的设置
                    settings.novelaiApiKey = originalApiKey;
                    settings.novelaiModel = originalModel;
                    ...
                    // 在测试提示词旁边显示失败状态
                    $testPromptInput.after('<span id="test-status" style="color: red; margin-left: 10px;">测试失败</span>');
                    setTimeout(() => $('#test-status').remove(), 3000);
                } finally {
