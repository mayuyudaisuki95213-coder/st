...
highly detailed, masterpiece, best quality
     ...
     * @returns {Promise<string>} - 返回生成的图像URL或base64数据
     */
    async function callNovelAIApi(prompt, negativePrompt) {
        const config = settings.novelaiConfig;

        if (!config.enabled || !config.apiKey) {
            throw new Error("请先启用并配置 NovelAI API（API Key）");
        }
        ...
        const params = {
            ...
            parameters: {
                ...
                seed: config.seed || -1,
                qualityToggle: config.qualityToggle !== false, // 默认启用
                ucPreset: config.ucPreset || 0,
                // 添加其他参数
                steps: config.steps || 28,
                scale: config.scale || 5,
                sampler: config.sampler || 'k_dpmpp_2m',
                width: config.width || 832,
                height: config.height || 1216,
                // 如果有额外的UC参数，添加到负面提示词中
                uc: config.uc || ''
            }
        };

        // 如果设置了使用全局负面提示词，则合并
        if (config.useGlobalNegative && settings.globalNegative) {
            params.parameters.negative_prompt = negativePrompt ?
                `${negativePrompt}, ${settings.globalNegative}` : settings.globalNegative;
        }

        const url = config.baseUrl ? config.baseUrl.replace(/\/$/, '') + '/ai/generate-image' : 'https://api.novelai.net/ai/generate-image';
        const headers = {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${config.apiKey}`,
        ...
        addLog('NOVELAI_API', `请求 NovelAI API: ${url}`);
        addLog('NOVELAI_API', `参数: Model=${params.model}, Steps=${params.parameters.steps}, Scale=${params.parameters.scale}`);
        ...
        try {
            ...
            addLog('NOVELAI_API', `图像生成成功，大小: ${imageData.byteLength} bytes`);
            return imageUrl;
        } catch (error) {
            addLog('ERROR', `调用 NovelAI API 失败: ${error.message}`);
            throw error;
        }
    }
    ...
    async function handleGeneration(state) {
        ...
        const finalNegativePrompt = settings.globalNegative || '';
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                // 调用 NovelAI API
                const imageUrl = await callNovelAIApi(finalPrompt, finalNegativePrompt);
        ...
        const html = `
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div id="sd-tab-basic" class="sd-tab-content active">
                    ...
                    <button id="sd-test-api" class="sd-btn-secondary" style="width:100%; margin-top:10px;">🧪 测试LLM API连接</button>

                    <hr style="border: none; border-top: 1px solid rgba(255,255,255,0.1); margin: 20px 0;">

                    <h4 style="margin-bottom:15px;">🎨 NovelAI 配置</h4>

                    <div style="margin-bottom: 12px;">
                        <label style="display: flex; align-items: center; gap: 8px;">
                            <input type="checkbox" id="sd-nai-enabled" ${settings.novelaiConfig.enabled ? 'checked' : ''}>
                            <span style="font-weight: bold;">启用 NovelAI API</span>
                        </label>
                        <small style="color: #888; display: block; margin-left: 24px; margin-top: 4px;">
                            启用后将直接连接 NovelAI API 生成图片
                        </small>
                    </div>

                    <div class="sd-api-row">
                        <label>API Key</label>
                        <input type="password" id="sd-nai-key" class="text_pole" placeholder="输入 NovelAI API Key" value="${settings.novelaiConfig.apiKey}">
                    </div>
                    <div class="sd-api-row">
                        <label>Base URL</label>
                        <input type="text" id="sd-nai-base-url" class="text_pole" placeholder="https://api.novelai.net" value="${settings.novelaiConfig.baseUrl}">
                    </div>
                    <div class="sd-api-row">
                        <label>模型</label>
                        <select id="sd-nai-model" class="text_pole">
                            <option value="nai-diffusion-3" ${settings.novelaiConfig.model === 'nai-diffusion-3' ? 'selected' : ''}>nai-diffusion-3</option>
                            <option value="safe-diffusion" ${settings.novelaiConfig.model === 'safe-diffusion' ? 'selected' : ''}>safe-diffusion</option>
                            <option value="nai-diffusion" ${settings.novelaiConfig.model === 'nai-diffusion' ? 'selected' : ''}>nai-diffusion</option>
                            <option value="furry" ${settings.novelaiConfig.model === 'furry' ? 'selected' : ''}>furry</option>
                        </select>
                    </div>
                    <div class="sd-api-row">
                        <label>采样器</label>
                        <select id="sd-nai-sampler" class="text_pole">
                            <option value="k_euler" ${settings.novelaiConfig.sampler === 'k_euler' ? 'selected' : ''}>Euler</option>
                            <option value="k_euler_ancestral" ${settings.novelaiConfig.sampler === 'k_euler_ancestral' ? 'selected' : ''}>Euler A</option>
                            <option value="k_dpmpp_2m" ${settings.novelaiConfig.sampler === 'k_dpmpp_2m' ? 'selected' : ''}>DPM++ 2M</option>
                            <option value="k_dpmpp_2m_ancestral" ${settings.novelaiConfig.sampler === 'k_dpmpp_2m_ancestral' ? 'selected' : ''}>DPM++ 2M A</option>
                            <option value="k_dpmpp_sde" ${settings.novelaiConfig.sampler === 'k_dpmpp_sde' ? 'selected' : ''}>DPM++ SDE</option>
                        </select>
                    </div>
                    <div class="sd-api-row">
                        <label>步数</label>
                        <input type="number" id="sd-nai-steps" class="text_pole" value="${settings.novelaiConfig.steps}" min="1" max="50" step="1">
                    </div>
                    <div class="sd-api-row">
                        <label>CFG Scale</label>
                        <input type="number" id="sd-nai-scale" class="text_pole" value="${settings.novelaiConfig.scale}" min="1" max="20" step="0.5">
                    </div>
                    <div class="sd-api-row">
                        <label>宽度</label>
                        <input type="number" id="sd-nai-width" class="text_pole" value="${settings.novelaiConfig.width}" min="64" max="2048" step="64">
                    </div>
                    <div class="sd-api-row">
                        <label>高度</label>
                        <input type="number" id="sd-nai-height" class="text_pole" value="${settings.novelaiConfig.height}" min="64" max="2048" step="64">
                    </div>
                    <div class="sd-api-row">
                        <label>种子</label>
                        <input type="number" id="sd-nai-seed" class="text_pole" value="${settings.novelaiConfig.seed}" min="-1" max="999999999" step="1">
                    </div>
                    <div class="sd-api-row">
                        <label>
                            <input type="checkbox" id="sd-nai-quality-toggle" ${settings.novelaiConfig.qualityToggle ? 'checked' : ''}>
                            质量切换
                        </label>
                    </div>
                    <div class="sd-api-row">
                        <label>UC 预设</label>
                        <select id="sd-nai-uc-preset" class="text_pole">
                            <option value="0" ${settings.novelaiConfig.ucPreset === 0 ? 'selected' : ''}>0 (默认)</option>
                            <option value="1" ${settings.novelaiConfig.ucPreset === 1 ? 'selected' : ''}>1 (轻微修正)</option>
                            <option value="2" ${settings.novelaiConfig.ucPreset === 2 ? 'selected' : ''}>2 (标准修正)</option>
                        </select>
                    </div>
                    <div class="sd-api-row">
                        <label>
                            <input type="checkbox" id="sd-nai-use-global-negative" ${settings.novelaiConfig.useGlobalNegative ? 'checked' : ''}>
                            使用全局负面提示词
                        </label>
                    </div>
                    <div class="sd-api-row">
                        <label>UC (额外负面)</label>
                        <input type="text" id="sd-nai-uc" class="text_pole" value="${settings.novelaiConfig.uc}" placeholder="额外的负面提示词">
                    </div>
                    <button id="sd-test-nai-api" class="sd-btn-secondary" style="width:100%; margin-top:10px;">🧪 测试 NovelAI API</button>
                </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-test-api').on('click', async function () {
                ...
                try {
                    ...
                    toastr.success('✅ LLM API连接测试成功！');
                } catch (e) {
                    toastr.error(`❌ LLM API测试失败: ${e.message}`);
                } finally {
                    $btn.prop('disabled', false).text('🧪 测试LLM API连接');
                }
            });

            // 测试 NovelAI API
            $('#sd-test-nai-api').on('click', async function () {
                const $btn = $(this);
                const apiKey = $('#sd-nai-key').val();
                const baseUrl = $('#sd-nai-base-url').val() || 'https://api.novelai.net';
                ...
                try {
                    // 临时保存当前配置以便测试
                    const oldConfig = {...settings.novelaiConfig};

                    // 更新配置为测试值
                    settings.novelaiConfig = {
                        ...settings.novelaiConfig,
                        enabled: true,
                        apiKey: apiKey,
                        baseUrl: baseUrl,
                        model: $('#sd-nai-model').val(),
                        sampler: $('#sd-nai-sampler').val(),
                        steps: parseInt($('#sd-nai-steps').val()) || 28,
                        scale: parseFloat($('#sd-nai-scale').val()) || 5,
                        width: parseInt($('#sd-nai-width').val()) || 832,
                        height: parseInt($('#sd-nai-height').val()) || 1216,
                        seed: parseInt($('#sd-nai-seed').val()) || -1,
                        qualityToggle: $('#sd-nai-quality-toggle').is(':checked'),
                        ucPreset: parseInt($('#sd-nai-uc-preset').val()) || 0,
                        uc: $('#sd-nai-uc').val() || '',
                        useGlobalNegative: $('#sd-nai-use-global-negative').is(':checked')
                    };

                    // 使用简单的提示词进行测试
                    await callNovelAIApi('1girl, smiling', 'lowres, bad anatomy');

                    // 恢复原始配置
                    settings.novelaiConfig = oldConfig;

                    toastr.success('✅ NovelAI API连接测试成功！');
                } catch (e) {
                    toastr.error(`❌ NovelAI API测试失败: ${e.message}`);
                } finally {
                    $btn.prop('disabled', false).text('🧪 测试 NovelAI API');
                }
            });
            ...
            $('#sd-save').on('click', () => {
                ...
                // NovelAI配置
                settings.novelaiConfig.enabled = $('#sd-nai-enabled').is(':checked');
                settings.novelaiConfig.apiKey = $('#sd-nai-key').val();
                settings.novelaiConfig.baseUrl = $('#sd-nai-base-url').val() || 'https://api.novelai.net';
                settings.novelaiConfig.model = $('#sd-nai-model').val();
                settings.novelaiConfig.sampler = $('#sd-nai-sampler').val();
                settings.novelaiConfig.steps = parseInt($('#sd-nai-steps').val()) || 28;
                settings.novelaiConfig.scale = parseFloat($('#sd-nai-scale').val()) || 5;
                settings.novelaiConfig.width = parseInt($('#sd-nai-width').val()) || 832;
                settings.novelaiConfig.height = parseInt($('#sd-nai-height').val()) || 1216;
                settings.novelaiConfig.seed = parseInt($('#sd-nai-seed').val()) || -1;
                settings.novelaiConfig.qualityToggle = $('#sd-nai-quality-toggle').is(':checked');
                settings.novelaiConfig.ucPreset = parseInt($('#sd-nai-uc-preset').val()) || 0;
                settings.novelaiConfig.uc = $('#sd-nai-uc').val() || '';
                settings.novelaiConfig.useGlobalNegative = $('#sd-nai-use-global-negative').is(':checked');
