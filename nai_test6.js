...
highly detailed, masterpiece, best quality
    ...
    async function callNovelAIApi(prompt) {
        ...
        // 确保必要的参数存在
        if (!parameters.width) parameters.width = settings.novelaiParameters.width || 1792;
        if (!parameters.height) parameters.height = settings.novelaiParameters.height || 1024;
        if (!parameters.steps) parameters.steps = settings.novelaiParameters.steps || 28;
        if (!parameters.scale) parameters.scale = settings.novelaiParameters.scale || 11;
