...
highly detailed, masterpiece, best quality
    ...
    const DEFAULT_SETTINGS = {
        ...
        // NovelAI API 配置
        novelaiEnabled: false,             // NovelAI 生图功能开关
        novelaiApiKey: '',                 // NovelAI API Key
        novelaiModel: 'nai-diffusion-4-5-full', // NovelAI 模型名称
        novelaiParams: {                   // NovelAI 生图参数
            width: 1216,
            height: 832,
            scale: 6,
            steps: 28,
        ...
        novelaiPrefix: 'best quality, amazing quality, very aesthetic, absurdres,', // NovelAI 正面提示词前缀
        novelaiNegative: 'lowres, bad anatomy, bad hands, missing fingers, extra digits, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry', // NovelAI 负面提示词
    ...
    /* NovelAI 测试状态样式 */
    .success-status { color: green !important; font-weight: bold; }
    .error-status { color: red !important; font-weight: bold; }
    `;
    ...
    function initGlobalListeners() {
        ...
        $chat.on('click', '.sd-ui-image', function (e) {
            const $img = $(this);
            ...
            // 点击图片左侧 (clickX < width/2)：切换显示上一张图片
            if (clickX < imgWidth / 2) {
                if (curIdx > 0) {
                    updateWrapperView(s.$wrap, s.images, curIdx - 1);
                }
            }
            // 点击图片右侧 (clickX > width/2)
            else {
                // 如果当前不是最新图：切换显示下一张图片
                if (curIdx < totalImages - 1) {
                    updateWrapperView(s.$wrap, s.images, curIdx + 1);
                }
                // 如果当前已经是最新图：触发 NAI 重新生图 (Reroll)
                else {
                    handleGeneration(s);
                }
            }
    ...
    async function handleGeneration(state) {
        ...
        // 根据设置决定使用 NovelAI 还是酒馆自带生图
        if (settings.novelaiEnabled) {
            // 使用 NovelAI API
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    if (attempt > 1) {
                        state.el.msg.text(`⏳ 重试中 (${attempt}/${MAX_RETRIES})...`);
                        addLog('GENERATION', `第${attempt}次重试 NovelAI 生图...`);
                    }

                    // 构建最终提示词
                    const finalPrompt = `${settings.novelaiPrefix ? settings.novelaiPrefix + ' ' : ''}${state.prompt}`.replace(/\s+/g, ' ').trim();

                    // 根据设置决定是否启用超时
                    let result;
                    if (settings.timeoutEnabled) {
                        result = await withTimeout(callNovelAIForImage(finalPrompt), settings.timeoutSeconds * 1000);
                    } else {
                        result = await callNovelAIForImage(finalPrompt);
                    }

                    if (result) {
                        state.el.msg.text('✅ 成功');
                        // 如果是 base64 图片，需要通过酒馆的图片保存机制处理
                        let uniqueImages;
                        if (result.startsWith('data:image/')) {
                            // 使用新的uploadImage函数处理 base64 图片并上传到服务器
                            try {
                                // 从 base64 数据中提取扩展名
                                const ext = result.startsWith('data:image/png') ? 'png' :
                                          result.startsWith('data:image/jpeg') ? 'jpg' : 'png';
                                const fileName = `NAI_${Date.now()}_${state.blockIdx}.${ext}`;

                                // 上传图片到服务器
                                const imageUrl = await uploadImage(result, fileName);

                                // 使用上传后的URL更新图片列表
                                uniqueImages = [...new Set([...state.images, imageUrl])];

                                // 更新对话历史
                                await updateChatDataWithImage(state.mesId, state.blockIdx, state.prompt, uniqueImages);
                                ...
                                console.error('上传 NovelAI 图片失败:', uploadError);
                                // 如果上传失败，仍然使用 base64 数据，但不存储到文本
                                uniqueImages = [...new Set([...state.images, result])];
                                // 直接更新 UI，不更新聊天文本
                                setTimeout(() => {
                                    const $newWrap = $(`.mes[mesid="${state.mesId}"] .sd-ui-wrap[data-block-idx="${state.blockIdx}"]`);
                                    if ($newWrap.length) {
                                        updateWrapperView($newWrap, uniqueImages, uniqueImages.length - 1);
                                        // 更新 wrap 的 data-images 属性，但不更新聊天文本
                                        $newWrap.attr('data-images', encodeURIComponent(JSON.stringify(uniqueImages)));
                                    }
                                }, 200);
                            }
                        } else {
                        ...
                        if (attempt < MAX_RETRIES) {
                            addLog('GENERATION', `第${attempt}次 NovelAI 尝试无结果，${RETRY_DELAY_MS}ms后重试...`);
                            await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
                        }
                    }
                } catch (err) {
                    console.error(`NovelAI Generation attempt ${attempt} error:`, err);
                    lastError = err;

                    if (attempt < MAX_RETRIES) {
                        addLog('GENERATION', `第${attempt}次 NovelAI 尝试失败: ${err.message}，${RETRY_DELAY_MS}ms后重试...`);
                        await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
                    }
                }
            }
            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    ...
                    if (trimmedUrls.length > 0) {
                            ...
                            if (url.startsWith('data:image/')) {
                                try {
                                    ...
                                    const ext = url.startsWith('data:image/png') ? 'png' :
                                              url.startsWith('data:image/jpeg') ? 'jpg' : 'png';
                                    const fileName = `SD_${Date.now()}_${state.blockIdx}.${ext}`;

                                    // 上传图片到服务器
                                    const imageUrl = await uploadImage(url, fileName);
                        ...
                        const uniqueImages = [...new Set([...state.images, ...processedUrls])];
     ...
     */
    async function updateChatDataWithImage(mesId, blockIndex, prompt, images) {
        // 获取当前对话上下文
        const context = SillyTavern.getContext();
        const chat = context.chat;

        if (!chat || !chat[mesId]) {
            console.error('无法找到对应的消息');
            return;
        }
        ...
        if (matches[blockIndex]) {
            ...
            // 构建新的块内容，包含所有图片URL
            let newInnerContent = prompt;

            // 添加所有图片URL（只添加持久化的URL，不添加base64数据）
            const persistentImages = images.filter(img => !img.startsWith('data:image/'));
            if (persistentImages.length > 0) {
                newInnerContent += '\n' + persistentImages.join('\n');
            }
            ...
            // 替换原内容中的对应块
            const newContent = originalContent.substring(0, match.index) + newBlock + originalContent.substring(match.index + match[0].length);
    ...
    function updateWrapperView($wrap, images, idx) {
        ...
        if (count === 0) {
            ...
            $ph.hide();
            // 检查是否是 base64 图片（NovelAI 返回的格式）
            if (images[idx] && images[idx].startsWith('data:image/')) {
                $img.attr('src', images[idx]); // 直接使用 base64 数据
            } else {
                $img.attr('src', encodeImageUrl(images[idx])); // 使用 URL 编码
            }
            $img.show();
            $left.toggle(idx > 0); $del.show();
    ...
    async function processSequentialQueue() {
        ...
        while (sequentialQueue.length > 0) {
            ...
            const state = {
                ...
                images: JSON.parse(decodeURIComponent($currentWrap.attr('data-images') || '[]')),
                el: {
    ...
    function parseBlockContent(raw) {
        ...
        // 合并两种类型的图片
        const urlImages = (text.match(urlRegex) || []).map(url => url.trim());
        const base64Images = (text.match(base64Regex) || []).map(img => img.trim());
        const images = [...urlImages, ...base64Images];

        // 从文本中移除所有图片链接（包括 base64）
        let prompt = text
            .replace(urlRegex, '')
            .replace(base64Regex, '')
            .replace(NO_GEN_FLAG, '')
            .replace(SCHEDULED_FLAG, '')
            .trim();
    ...
    function rebuildBlockString(prompt, images, prevent, scheduled) {
        let res = prompt;
        if (images.length > 0) {
            // 只保存非 base64 格式的图片（URL 类型），避免将大型 base64 数据存储在文本中
            const urlImages = images.filter(img => !img.startsWith('data:image/'));
        ...
        <div class="sd-ui-container">
            <div class="sd-ui-wrap" data-prompt="${encodeURIComponent(prompt)}" data-images="${encodeURIComponent(JSON.stringify(images))}" data-prevent-auto="${prevent}" data-block-idx="${blockIdx}" data-cur-idx="${initIdx}" data-scheduled="${isScheduled}">
                ...
                <div class="sd-ui-viewport">
                    ...
                    <img class="sd-ui-image" src="${has ? encodeImageUrl(images[initIdx]) : ''}" style="display:${has ? 'block' : 'none'}" data-block-idx="${blockIdx}" data-mes-id="" />
    ...
    function openSettingsPopup() {
        ...
        const html = `
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div class="sd-tab-nav">
                    ...
                    <div class="sd-tab-btn" data-tab="novelai">NovelAI</div>
                ...
                <div id="sd-tab-novelai" class="sd-tab-content">
                    <h4 style="margin-top:0; margin-bottom:15px;">NovelAI 生图设置</h4>

                    <div style="margin-bottom: 12px;">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                <input type="checkbox" id="sd-novelai-en" ${settings.novelaiEnabled ? 'checked' : ''}>
                                <span style="font-weight: bold;">启用 NovelAI 生图</span>
                            </label>
                        </div>
                        <small style="color: #888; display: block; margin-left: 24px; margin-top: 4px;">
                            开启后将使用 NovelAI API 生成图片，而不是酒馆自带的生图功能
                        </small>
                    </div>

                    <div class="sd-api-row">
                        <label>API Key</label>
                        <input type="password" id="sd-novelai-key" class="text_pole" placeholder="输入 NovelAI API Key" value="${settings.novelaiApiKey}">
                    </div>

                    <div class="sd-api-row">
                        <label>模型</label>
                        <select id="sd-novelai-model" class="text_pole">
                            <option value="nai-diffusion-4-5-full" ${settings.novelaiModel === 'nai-diffusion-4-5-full' ? 'selected' : ''}>nai-diffusion-4-5-full</option>
                            <option value="nai-diffusion-3" ${settings.novelaiModel === 'nai-diffusion-3' ? 'selected' : ''}>nai-diffusion-3</option>
                            <option value="safe-diffusion" ${settings.novelaiModel === 'safe-diffusion' ? 'selected' : ''}>safe-diffusion</option>
                            <option value="furry-3" ${settings.novelaiModel === 'furry-3' ? 'selected' : ''}>furry-3</option>
                        </select>
                    </div>

                    <div style="margin-top: 20px; padding: 15px; background: linear-gradient(145deg, #252530, #1e1e24); border-radius: 8px; box-shadow: 3px 3px 6px var(--nm-shadow-dark), -2px -2px 5px var(--nm-shadow-light);">
                        <h4 style="margin-top:0; margin-bottom:15px;">生图参数</h4>

                        <div class="sd-api-row">
                            <label>分辨率预设</label>
                            <select id="sd-novelai-resolution" class="text_pole">
                                <option value="512x512" ${settings.novelaiParams.width === 512 && settings.novelaiParams.height === 512 ? 'selected' : ''}>512x512 (1:1,icons, profile pictures)</option>
                                <option value="600x600" ${settings.novelaiParams.width === 600 && settings.novelaiParams.height === 600 ? 'selected' : ''}>600x600 (1:1,icons,profile pictures)</option>
                                <option value="512x768" ${settings.novelaiParams.width === 512 && settings.novelaiParams.height === 768 ? 'selected' : ''}>512x768 (2:3, vertical character card)</option>
                                <option value="768x512" ${settings.novelaiParams.width === 768 && settings.novelaiParams.height === 512 ? 'selected' : ''}>768x512 (3:2,horizontal 35-mm movie film)</option>
                                <option value="540x960" ${settings.novelaiParams.width === 540 && settings.novelaiParams.height === 960 ? 'selected' : ''}>540x960 (9:16,vertical wallpaper)</option>
                                <option value="960x540" ${settings.novelaiParams.width === 960 && settings.novelaiParams.height === 540 ? 'selected' : ''}>960x540 (16:9,horizontal wallpaper)</option>
                                <option value="720x1280" ${settings.novelaiParams.width === 720 && settings.novelaiParams.height === 1280 ? 'selected' : ''}>720x1280 (9:16, 720p,vertical wallpaper)</option>
                                <option value="1280x720" ${settings.novelaiParams.width === 1280 && settings.novelaiParams.height === 720 ? 'selected' : ''}>1280x720 (16:9,720p,horizontal wallpaper)</option>
                                <option value="1088x1920" ${settings.novelaiParams.width === 1088 && settings.novelaiParams.height === 1920 ? 'selected' : ''}>1088x1920 (9:16,1080p,vertical wallpaper)</option>
                                <option value="1920x1088" ${settings.novelaiParams.width === 1920 && settings.novelaiParams.height === 1088 ? 'selected' : ''}>1920x1088 (16:9,1080p,horizontal wallpaper)</option>
                                <option value="1024x1024" ${settings.novelaiParams.width === 1024 && settings.novelaiParams.height === 1024 ? 'selected' : ''}>1024x1024 (1:1,SDXL)</option>
                                <option value="896x1152" ${settings.novelaiParams.width === 896 && settings.novelaiParams.height === 1152 ? 'selected' : ''}>896x1152 (7:9,SDXL)</option>
                                <option value="1152x896" ${settings.novelaiParams.width === 1152 && settings.novelaiParams.height === 896 ? 'selected' : ''}>1152x896 (9:7,SDXL)</option>
                                <option value="832x1216" ${settings.novelaiParams.width === 832 && settings.novelaiParams.height === 1216 ? 'selected' : ''}>832x1216 (13:19,SDXL)</option>
                                <option value="1216x832" ${settings.novelaiParams.width === 1216 && settings.novelaiParams.height === 832 ? 'selected' : ''}>1216x832 (19:13,SDXL)</option>
                                <option value="768x1344" ${settings.novelaiParams.width === 768 && settings.novelaiParams.height === 1344 ? 'selected' : ''}>768x1344 (3:4,SDXL)</option>
                                <option value="1344x768" ${settings.novelaiParams.width === 1344 && settings.novelaiParams.height === 768 ? 'selected' : ''}>1344x768 (4:3,SDXL)</option>
                                <option value="640x1536" ${settings.novelaiParams.width === 640 && settings.novelaiParams.height === 1536 ? 'selected' : ''}>640x1536 (10:24,SDXL)</option>
                                <option value="1536x640" ${settings.novelaiParams.width === 1536 && settings.novelaiParams.height === 640 ? 'selected' : ''}>1536x640 (24:10,SDXL)</option>
                                <option value="1024x1536" ${settings.novelaiParams.width === 1024 && settings.novelaiParams.height === 1536 ? 'selected' : ''}>1024x1536 (2:3,ChatGPT)</option>
                                <option value="1536x1024" ${settings.novelaiParams.width === 1536 && settings.novelaiParams.height === 1024 ? 'selected' : ''}>1536x1024 (3:2,ChatGPT)</option>
                                <option value="1024x1792" ${settings.novelaiParams.width === 1024 && settings.novelaiParams.height === 1792 ? 'selected' : ''}>1024x1792 (4:7，DALL-E)</option>
                                <option value="1792x1024" ${settings.novelaiParams.width === 1792 && settings.novelaiParams.height === 1024 ? 'selected' : ''}>1792x1024 (7:4,DALL-E)</option>
                            </select>
                        </div>

                        <div class="sd-api-row">
                            <label>CFG Scale</label>
                            <input type="number" id="sd-novelai-scale" class="text_pole" value="${settings.novelaiParams.scale}" min="1" max="20" step="0.5">
                        </div>

                        <div class="sd-api-row">
                            <label>步数</label>
                            <input type="number" id="sd-novelai-steps" class="text_pole" value="${settings.novelaiParams.steps}" min="1" max="50" step="1">
                        </div>

                        <div class="sd-api-row">
                            <label>采样器</label>
                            <select id="sd-novelai-sampler" class="text_pole">
                                <option value="k_euler" ${settings.novelaiParams.sampler === 'k_euler' ? 'selected' : ''}>Euler</option>
                                <option value="k_euler_ancestral" ${settings.novelaiParams.sampler === 'k_euler_ancestral' ? 'selected' : ''}>Euler a</option>
                                <option value="k_dpmpp_2m" ${settings.novelaiParams.sampler === 'k_dpmpp_2m' ? 'selected' : ''}>DPM++ 2M</option>
                                <option value="k_dpmpp_2m_ancestral" ${settings.novelaiParams.sampler === 'k_dpmpp_2m_ancestral' ? 'selected' : ''}>DPM++ 2M a</option>
                                <option value="k_dpmpp_sde" ${settings.novelaiParams.sampler === 'k_dpmpp_sde' ? 'selected' : ''}>DPM++ SDE</option>
                                <option value="ddim" ${settings.novelaiParams.sampler === 'ddim' ? 'selected' : ''}>DDIM</option>
                            </select>
                        </div>

                        <div class="sd-api-row">
                            <label>调度器</label>
                            <select id="sd-novelai-scheduler" class="text_pole">
                                <option value="karras" ${settings.novelaiParams.scheduler === 'karras' ? 'selected' : ''}>Karras</option>
                                <option value="normal" ${settings.novelaiParams.scheduler === 'normal' ? 'selected' : ''}>Normal</option>
                                <option value="exponential" ${settings.novelaiParams.scheduler === 'exponential' ? 'selected' : ''}>Exponential</option>
                                <option value="polyexponential" ${settings.novelaiParams.scheduler === 'polyexponential' ? 'selected' : ''}>Polyexponential</option>
                            </select>
                        </div>

                        <div class="sd-api-row">
                            <label>种子</label>
                            <input type="number" id="sd-novelai-seed" class="text_pole" value="${settings.novelaiParams.seed}" min="-1" max="4294967295">
                            <small style="color: #888; display: block; margin-top: 4px;">-1 表示随机种子</small>
                        </div>

                        <div style="display: flex; gap: 15px; flex-wrap: wrap; margin-top: 10px;">
                            <label style="display: flex; align-items: center; gap: 5px;">
                                <input type="checkbox" id="sd-novelai-quality-toggle" ${settings.novelaiParams.qualityToggle ? 'checked' : ''}>
                                <span>Quality Toggle</span>
                            </label>
                            <label style="display: flex; align-items: center; gap: 5px;">
                                <input type="checkbox" id="sd-novelai-auto-smea" ${settings.novelaiParams.autoSmea ? 'checked' : ''}>
                                <span>Auto Smea</span>
                            </label>
                            <label style="display: flex; align-items: center; gap: 5px;">
                                <input type="checkbox" id="sd-novelai-sm" ${settings.novelaiParams.sm ? 'checked' : ''}>
                                <span>SM</span>
                            </label>
                            <label style="display: flex; align-items: center; gap: 5px;">
                                <input type="checkbox" id="sd-novelai-sm-dyn" ${settings.novelaiParams.sm_dyn ? 'checked' : ''}>
                                <span>SM Dyn</span>
                            </label>
                            <label style="display: flex; align-items: center; gap: 5px;">
                                <input type="checkbox" id="sd-novelai-decrisper" ${settings.novelaiParams.decrisper ? 'checked' : ''}>
                                <span>Decrisper</span>
                            </label>
                            <label style="display: flex; align-items: center; gap: 5px;">
                                <input type="checkbox" id="sd-novelai-variety-boost" ${settings.novelaiParams.variety_boost ? 'checked' : ''}>
                    ...
                    <div style="margin-top: 20px; padding: 15px; background: linear-gradient(145deg, #252530, #1e1e24); border-radius: 8px; box-shadow: 3px 3px 6px var(--nm-shadow-dark), -2px -2px 5px var(--nm-shadow-light);">
                        <h4 style="margin-top:0; margin-bottom:15px;">提示词设置</h4>

                        <label style="display:block; margin-bottom:5px;">正面提示词前缀</label>
                        <textarea id="sd-novelai-prefix" class="text_pole" rows="3" style="width:100%">${settings.novelaiPrefix}</textarea>

                        <label style="margin-top:15px; display:block; margin-bottom:5px;">负面提示词</label>
                        <textarea id="sd-novelai-negative" class="text_pole" rows="4" style="width:100%">${settings.novelaiNegative}</textarea>
                    </div>

                    <div style="margin-top: 15px; display: flex; align-items: center;">
                        <label style="display:block; margin-right: 10px; min-width: 80px;">测试提示词</label>
                        <input type="text" id="sd-novelai-test-prompt" class="text_pole" placeholder="输入测试提示词，例如：1girl" value="1girl" style="flex: 1; margin-right: 10px;">
                        <span id="sd-novelai-test-status" style="display: none; font-weight: bold;"></span>
                    </div>

                    <button id="sd-test-novelai" class="sd-btn-secondary" style="width:100%; margin-top:15px;">🧪 测试 NovelAI 连接</button>

                    <div id="sd-novelai-test-result" style="margin-top:15px; display:none; text-align: center;">
                        <label style="display:block; margin-bottom:5px;">测试结果</label>
                        <img id="sd-novelai-test-image" style="max-width:100%; max-height:300px; border-radius:8px; box-shadow: 3px 3px 8px var(--nm-shadow-dark), -2px -2px 6px var(--nm-shadow-light);" />
                    </div>
                </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-save').on('click', () => {
                ...
                // NovelAI 设置
                settings.novelaiEnabled = $('#sd-novelai-en').is(':checked');
                settings.novelaiApiKey = $('#sd-novelai-key').val();
                settings.novelaiModel = $('#sd-novelai-model').val();
                settings.novelaiPrefix = $('#sd-novelai-prefix').val();
                settings.novelaiNegative = $('#sd-novelai-negative').val();

                // NovelAI 参数
                const resolutionValue = $('#sd-novelai-resolution').val();
                const [width, height] = resolutionValue.split('x').map(Number);

                settings.novelaiParams = {
                    width: width || 1216,
                    height: height || 832,
            ...
            // 为 NovelAI 板块添加实时保存功能
            function setupNovelAIRealTimeSave() {
                // 监听 NovelAI 启用状态变化
                $('#sd-novelai-en').on('change', function() {
                    settings.novelaiEnabled = $(this).is(':checked');
                    saveSettings();
                });

                // 监听 API Key 变化
                $('#sd-novelai-key').on('input change', function() {
                    settings.novelaiApiKey = $(this).val();
                    saveSettings();
                });

                // 监听模型选择变化
                $('#sd-novelai-model').on('change', function() {
                    settings.novelaiModel = $(this).val();
                    saveSettings();
                });

                // 监听分辨率变化
                $('#sd-novelai-resolution').on('change', function() {
                    const resolutionValue = $(this).val();
                    const [width, height] = resolutionValue.split('x').map(Number);
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        width: width || 1216,
                ...
                // 监听 Scale 变化
                $('#sd-novelai-scale').on('input change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        scale: parseFloat($(this).val()) || 6
                    };
                    saveSettings();
                });

                // 监听 Steps 变化
                $('#sd-novelai-steps').on('input change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        steps: parseInt($(this).val()) || 28
                    };
                    saveSettings();
                });

                // 监听 Seed 变化
                $('#sd-novelai-seed').on('input change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        seed: parseInt($(this).val()) || -1
                    };
                    saveSettings();
                });

                // 监听 Sampler 变化
                $('#sd-novelai-sampler').on('change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        sampler: $(this).val()
                    };
                ...
                // 监听 Scheduler 变化
                $('#sd-novelai-scheduler').on('change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        scheduler: $(this).val()
                    };
                ...
                // 监听 Quality Toggle 变化
                $('#sd-novelai-quality-toggle').on('change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        qualityToggle: $(this).is(':checked')
                    };
                    saveSettings();
                });

                // 监听 Auto Smea 变化
                $('#sd-novelai-auto-smea').on('change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        autoSmea: $(this).is(':checked')
                    };
                    saveSettings();
                });

                // 监听 SM 变化
                $('#sd-novelai-sm').on('change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        sm: $(this).is(':checked')
                    };
                ...
                // 监听 SM Dyn 变化
                $('#sd-novelai-sm-dyn').on('change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        sm_dyn: $(this).is(':checked')
                    };
                    saveSettings();
                });

                // 监听 Decrisper 变化
                $('#sd-novelai-decrisper').on('change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        decrisper: $(this).is(':checked')
                    };
                    saveSettings();
                });

                // 监听 Variety Boost 变化
                $('#sd-novelai-variety-boost').on('change', function() {
                    settings.novelaiParams = {
                        ...settings.novelaiParams,
                        variety_boost: $(this).is(':checked')
                    };
                    saveSettings();
                });

                // 监听正面提示词前缀变化
                $('#sd-novelai-prefix').on('input', function() {
                    settings.novelaiPrefix = $(this).val();
                    saveSettings();
                });

                // 监听负面提示词变化
                $('#sd-novelai-negative').on('input', function() {
                    settings.novelaiNegative = $(this).val();
                    saveSettings();
                });

                // 监听测试提示词变化
                $('#sd-novelai-test-prompt').on('input', function() {
                    // 测试提示词不需要保存到设置中，仅用于测试
                });
            }

            // 初始化 NovelAI 实时保存功能
            setupNovelAIRealTimeSave();

            // 测试 NovelAI 连接
            $('#sd-test-novelai').on('click', async function () {
                const $btn = $(this);
                const $resultDiv = $('#sd-novelai-test-result');
                const $resultImg = $('#sd-novelai-test-image');
                const apiKey = $('#sd-novelai-key').val();
                const model = $('#sd-novelai-model').val();
                const testPrompt = $('#sd-novelai-test-prompt').val() || '1girl';

                if (!apiKey) {
                    toastr.warning('请先填写 NovelAI API Key');
                    return;
                }
                ...
                // 清除之前的测试状态并隐藏
                $('#sd-novelai-test-status').hide().removeClass('success-status error-status');

                try {
                    // 临时保存当前设置
                    const originalApiKey = settings.novelaiApiKey;
                    const originalModel = settings.novelaiModel;
                    const originalParams = { ...settings.novelaiParams };
                    const originalPrefix = settings.novelaiPrefix;
                    const originalNegative = settings.novelaiNegative;
                    ...
                    // 使用当前 UI 中设置的正面和负面提示词
                    settings.novelaiPrefix = $('#sd-novelai-prefix').val();
                    settings.novelaiNegative = $('#sd-novelai-negative').val();

                    // 合并正面提示词前缀和测试提示词作为完整的正面提示词
                    const combinedPrompt = [$('#sd-novelai-prefix').val(), testPrompt]
     ...
     * @param {string} prefix - 正面提示词前缀（可选，用于覆盖设置中的值）
     * @param {string} negative - 负面提示词（可选，用于覆盖设置中的值）
     * @returns {Promise<string>} - 返回图片的 base64 数据
     */
    async function callNovelAIForImage(prompt, params = {}, prefix, negative) {
        if (!settings.novelaiApiKey) {
            throw new Error("请先配置 NovelAI API Key");
        }
        ...
        // 使用传入的提示词或设置中的默认值
        const finalPrefix = prefix !== undefined ? prefix : settings.novelaiPrefix;
        const finalNegative = negative !== undefined ? negative : settings.novelaiNegative;

        // 构建请求体（参考 novel-draw.js 的实现）
        const requestBody = buildNovelAIRequestBodyForHelper({
            scene: prompt,
            characterPrompts: [], // 暂时为空，可根据需要扩展
            negativePrompt: finalNegative,
            params: finalParams
        });

        addLog('NOVELAI', `请求 NovelAI API: ${settings.novelaiModel}, 尺寸: ${finalParams.width}x${finalParams.height}`);
        ...
        try {
            ...
            if (!response.ok) {
                ...
                addLog('ERROR', `NovelAI API 响应错误: ${response.status} - ${errorText}`);
                throw new Error(`API Error ${response.status}: ${errorText}`);
            }

            // NovelAI API 返回的是 ZIP 格式的压缩包，需要解压获取图片
            const zipArrayBuffer = await response.arrayBuffer();
            const base64 = await extractImageFromZip(zipArrayBuffer);

            addLog('NOVELAI', `NovelAI 图片生成成功`);
            return `data:image/png;base64,${base64}`;
        } catch (error) {
            addLog('ERROR', `NovelAI API 调用失败: ${error.message}`);
            throw error;
        }
    }

    /**
     * 构建 NovelAI API 请求体（参考 novel-draw.js 的实现）
     */
    function buildNovelAIRequestBodyForHelper({ scene, characterPrompts, negativePrompt, params }) {
        const defaultParams = {
            width: 1216,
            height: 832,
            scale: 6,
        ...
        const width = params?.width ?? defaultParams.width;
        const height = params?.height ?? defaultParams.height;
        const seed = (params?.seed >= 0) ? params.seed : Math.floor(Math.random() * (0xFFFFFFFF + 1)); // MAX_SEED
        const modelName = params?.model ?? settings.novelaiModel;
        ...
        if (isV3) {
            ...
            return {
                ...
                parameters: {
                    width, height,
                    scale: params?.scale ?? defaultParams.scale,
                    seed,
                    sampler: params?.sampler ?? defaultParams.sampler,
                    noise_schedule: params?.scheduler ?? defaultParams.scheduler,
                    steps: params?.steps ?? defaultParams.steps,
                    n_samples: 1,
                    negative_prompt: String(allNegative || ''),
                    ucPreset: params?.ucPreset ?? defaultParams.ucPreset,
                    sm: params?.sm ?? defaultParams.sm,
                    sm_dyn: params?.sm_dyn ?? defaultParams.sm_dyn,
                    dynamic_thresholding: params?.decrisper ?? defaultParams.decrisper,
                },
            };
        }
        ...
        return {
            ...
            parameters: {
                ...
                n_samples: 1,
                ucPreset: params?.ucPreset ?? defaultParams.ucPreset,
                qualityToggle: params?.qualityToggle ?? defaultParams.qualityToggle,
                autoSmea: params?.autoSmea ?? defaultParams.autoSmea,
                cfg_rescale: params?.cfg_rescale ?? defaultParams.cfg_rescale,
    ...
    /**
     * 从 ZIP 数据中提取图片
     * @param {ArrayBuffer} zipData - ZIP 格式的二进制数据
     * @returns {Promise<string>} - 返回图片的 base64 数据
     */
    ...
    /**
     * 确保 JSZip 库已加载
     * @returns {Promise<any>} - 返回 JSZip 实例
     */
    async function ensureJSZip() {
        if (window.JSZip) return window.JSZip;

        // 创建一个 Promise 来等待 JSZip 加载
        return new Promise((resolve, reject) => {
    ...
    async function streamingGenerateImage(prompt) {
        ...
        try {
            if (settings.novelaiEnabled) {
                // 使用 NovelAI API
                const finalPrompt = `${settings.novelaiPrefix ? settings.novelaiPrefix + ' ' : ''}${prompt}`.replace(/\s+/g, ' ').trim();
                const result = await callNovelAIForImage(finalPrompt);

                // 如果是 base64 图片，需要上传到服务器
                if (result && result.startsWith('data:image/')) {
                    try {
                        // 从 base64 数据中提取扩展名
                        const ext = result.startsWith('data:image/png') ? 'png' :
                                  result.startsWith('data:image/jpeg') ? 'jpg' : 'png';
                        const fileName = `NAI_stream_${Date.now()}_${Math.floor(Math.random() * 1000)}.${ext}`;

                        // 上传图片到服务器
                        const imageUrl = await uploadImage(result, fileName);
                        return imageUrl;
                    } catch (uploadError) {
                        console.error('上传流式图片失败:', uploadError);
                        // 如果上传失败，返回原始base64数据
                        return result;
                    }
                }
                ...
                if (urls.length > 0) {
                    ...
                    if (firstUrl.startsWith('data:image/')) {
                        try {
                            // 从 base64 数据中提取扩展名
                            const ext = firstUrl.startsWith('data:image/png') ? 'png' :
                                      firstUrl.startsWith('data:image/jpeg') ? 'jpg' : 'png';
                            const fileName = `SD_stream_${Date.now()}_${Math.floor(Math.random() * 1000)}.${ext}`;

                            // 上传图片到服务器
                            const imageUrl = await uploadImage(firstUrl, fileName);
                            return imageUrl;
                        } catch (uploadError) {
                            console.error('上传流式图片失败:', uploadError);
                            // 如果上传失败，返回原始URL
    ...
    async function uploadImage(base64Data, fileName) {
        ...
        try {
            ...
            // 假设API返回图片路径，根据实际API响应调整
            // 通常返回类似 "user/images/MyExtension/fileName.png" 的路径
            if (result.path) {
                return result.path;
            } else if (result.url) {
                return result.url;
                ...
                // 如果API直接返回文件名，构造完整路径
                return `user/images/MyExtension/${fileName}`;
            }
        } catch (error) {
    ...
    function getCurrentCharacterName() {
        try {
            if (typeof SillyTavern !== 'undefined' && SillyTavern.name2) {
                return SillyTavern.name2;
            }
            if (typeof SillyTavern !== 'undefined' && SillyTavern.characters && typeof SillyTavern.this_chid !== 'undefined') {
                const character = SillyTavern.characters[SillyTavern.this_chid];
                if (character?.name) {
                    return character.name;
                }
