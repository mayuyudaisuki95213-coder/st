...
highly detailed, masterpiece, best quality
    ...
    const DEFAULT_SETTINGS = {
        ...
        // NovelAI API 配置
        novelaiConfig: {
            enabled: false,              // NovelAI API 开关
            apiUrl: 'https://image.novelai.net',  // NovelAI API 地址
            ...
            strength: 0.7,              // 强度
            parameters: {               // 其他参数
                qualityToggle: true,    // 质量切换
                sm: false,              // sm 参数
                sm_dyn: false,          // sm_dyn 参数
                dynamic_thresholding: false, // 动态阈值
                legacy: false,          // 传统模式
                add_original_image: false, // 添加原始图像
                cfg_rescale: 0,         // CFG 重缩放
                decrisper: false        // 去锐化
            }
        }
    };
    ...
    async function handleGeneration(state) {
        // 检查是否启用了NovelAI API
        if (settings.novelaiConfig.enabled) {
            // 使用NovelAI API
            await handleNovelAIGeneration(state);
        } else {
            // 使用原来的slash命令方式
    ...
    function parseBlockContent(raw) {
        ...
        // 保持原始URL格式，仅清理尾部空白
        const images = [
            ...(text.match(urlRegex) || []).map(url => url.trim()),
            ...(text.match(base64Regex) || []).map(url => url.trim())
        ];
    ...
    function openSettingsPopup() {
        ...
        const html = `
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div id="sd-tab-basic" class="sd-tab-content active">
                    ...
                    <h4 style="margin-bottom:15px;">NovelAI API 配置</h4>

                    <div style="margin-bottom: 12px;">
                        <div style="display: flex; align-items: center; gap: 8px;">
                            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                <input type="checkbox" id="sd-novelai-enabled" ${settings.novelaiConfig.enabled ? 'checked' : ''}>
                                <span style="font-weight: bold;">启用 NovelAI API</span>
                            </label>
                        </div>
                        <small style="color: #888; display: block; margin-left: 24px; margin-top: 4px;">
                            启用后将使用 NovelAI API 生成图片，禁用则使用原生slash命令
                        </small>
                    </div>

                    <div class="sd-api-row">
                        <label>API URL</label>
                        <input type="text" id="sd-novelai-url" class="text_pole" placeholder="https://image.novelai.net" value="${settings.novelaiConfig.apiUrl}">
                    </div>
                    <div class="sd-api-row">
                        <label>API Key</label>
                        <input type="password" id="sd-novelai-key" class="text_pole" placeholder="输入 NovelAI API Key" value="${settings.novelaiConfig.apiKey}">
                    </div>
                    <div class="sd-api-row">
                        <label>模型</label>
                        <input type="text" id="sd-novelai-model" class="text_pole" placeholder="nai-diffusion-3" value="${settings.novelaiConfig.model}">
                    </div>
                    <div class="sd-api-row">
                        <label>采样器</label>
                        <input type="text" id="sd-novelai-sampler" class="text_pole" placeholder="k_euler" value="${settings.novelaiConfig.sampler}">
                    </div>
                    <div class="sd-api-row">
                        <label>步数</label>
                        <input type="number" id="sd-novelai-steps" class="text_pole" value="${settings.novelaiConfig.steps}" min="1" max="100">
                    </div>
                    <div class="sd-api-row">
                        <label>引导尺度</label>
                        <input type="number" id="sd-novelai-scale" class="text_pole" value="${settings.novelaiConfig.scale}" min="0" max="20" step="0.1">
                    </div>
                    <div class="sd-api-row">
                        <label>种子</label>
                        <input type="number" id="sd-novelai-seed" class="text_pole" value="${settings.novelaiConfig.seed}" min="-1" max="999999999">
                        <small style="color: #888; display: block; margin-top: 4px;">-1 表示随机种子</small>
                    </div>
                    <div class="sd-api-row">
                        <label>噪声</label>
                        <input type="number" id="sd-novelai-noise" class="text_pole" value="${settings.novelaiConfig.noise}" min="0" max="1" step="0.01">
                    </div>
                    <div class="sd-api-row">
                        <label>强度</label>
                        <input type="number" id="sd-novelai-strength" class="text_pole" value="${settings.novelaiConfig.strength}" min="0" max="1" step="0.01">
                    </div>

                    <!-- NovelAI 高级参数 -->
                    <div style="margin-top: 15px; padding: 12px; background: linear-gradient(145deg, #252530, #1e1e24); border-radius: 8px; box-shadow: 3px 3px 6px var(--nm-shadow-dark), -2px -2px 5px var(--nm-shadow-light);">
                        <label style="display:block; margin-bottom:8px; font-weight:600;">🔧 高级参数</label>
                        <div class="sd-api-row">
                            <label style="flex: 0 0 150px;">质量切换</label>
                            <input type="checkbox" id="sd-novelai-quality-toggle" ${settings.novelaiConfig.parameters.qualityToggle ? 'checked' : ''}>
                        </div>
                        <div class="sd-api-row">
                            <label style="flex: 0 0 150px;">SM 参数</label>
                            <input type="checkbox" id="sd-novelai-sm" ${settings.novelaiConfig.parameters.sm ? 'checked' : ''}>
                        </div>
                        <div class="sd-api-row">
                            <label style="flex: 0 0 150px;">SM Dyn 参数</label>
                            <input type="checkbox" id="sd-novelai-sm-dyn" ${settings.novelaiConfig.parameters.sm_dyn ? 'checked' : ''}>
                        </div>
                        <div class="sd-api-row">
                            <label style="flex: 0 0 150px;">动态阈值</label>
                            <input type="checkbox" id="sd-novelai-dynamic-thresholding" ${settings.novelaiConfig.parameters.dynamic_thresholding ? 'checked' : ''}>
                        </div>
                        <div class="sd-api-row">
                            <label style="flex: 0 0 150px;">传统模式</label>
                            <input type="checkbox" id="sd-novelai-legacy" ${settings.novelaiConfig.parameters.legacy ? 'checked' : ''}>
                        </div>
                        <div class="sd-api-row">
                            <label style="flex: 0 0 150px;">CFG 重缩放</label>
                            <input type="number" id="sd-novelai-cfg-rescale" class="text_pole" value="${settings.novelaiConfig.parameters.cfg_rescale}" min="0" max="1" step="0.1">
                        </div>
                        <div class="sd-api-row">
                            <label style="flex: 0 0 150px;">添加原始图像</label>
                            <input type="checkbox" id="sd-novelai-add-original-image" ${settings.novelaiConfig.parameters.add_original_image ? 'checked' : ''}>
                        </div>
                    </div>
                </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-save').on('click', () => {
                ...
                // NovelAI 配置
                settings.novelaiConfig.enabled = $('#sd-novelai-enabled').is(':checked');
                settings.novelaiConfig.apiUrl = $('#sd-novelai-url').val();
                settings.novelaiConfig.apiKey = $('#sd-novelai-key').val();
                settings.novelaiConfig.model = $('#sd-novelai-model').val();
                settings.novelaiConfig.sampler = $('#sd-novelai-sampler').val();
                settings.novelaiConfig.steps = parseInt($('#sd-novelai-steps').val()) || 28;
                settings.novelaiConfig.scale = parseFloat($('#sd-novelai-scale').val()) || 5;
                settings.novelaiConfig.seed = parseInt($('#sd-novelai-seed').val()) || -1;
                settings.novelaiConfig.noise = parseFloat($('#sd-novelai-noise').val()) || 0.2;
                settings.novelaiConfig.strength = parseFloat($('#sd-novelai-strength').val()) || 0.7;

                // NovelAI 高级参数
                settings.novelaiConfig.parameters.qualityToggle = $('#sd-novelai-quality-toggle').is(':checked');
                settings.novelaiConfig.parameters.sm = $('#sd-novelai-sm').is(':checked');
                settings.novelaiConfig.parameters.sm_dyn = $('#sd-novelai-sm-dyn').is(':checked');
                settings.novelaiConfig.parameters.dynamic_thresholding = $('#sd-novelai-dynamic-thresholding').is(':checked');
                settings.novelaiConfig.parameters.legacy = $('#sd-novelai-legacy').is(':checked');
                settings.novelaiConfig.parameters.cfg_rescale = parseFloat($('#sd-novelai-cfg-rescale').val()) || 0;
                settings.novelaiConfig.parameters.add_original_image = $('#sd-novelai-add-original-image').is(':checked');
     ...
     * 调用 NovelAI API 生成图片
     * @param {string} prompt - 提示词
     * @param {Object} config - NovelAI 配置
     * @returns {Promise<string>} - 返回图片的base64数据URL
     */
    async function callNovelAIApi(prompt, config) {
        const url = config.apiUrl + '/ai/generate-image';
        ...
        const parameters = {
            ...
            sm_dyn: config.parameters.sm_dyn,
            dynamic_thresholding: config.parameters.dynamic_thresholding,
            cfg_rescale: config.parameters.cfg_rescale,
            decrisper: config.parameters.decrisper
        };

        // 如果是nai-diffusion-3模型，添加额外参数
        if (config.model === 'nai-diffusion-3') {
            Object.assign(parameters, {
                // nai-diffusion-3 特定参数
                add_original_image: config.parameters.add_original_image || false,
                controlnet_strength: 1, // 默认值
        ...
        addLog('NOVELAI_API', `请求NovelAI API: ${url}`);
        addLog('NOVELAI_API', `参数: ${JSON.stringify(payload.parameters)}`);
        ...
        try {
            ...
            if (!response.ok) {
                ...
                addLog('ERROR', `NovelAI API响应错误: ${response.status} - ${errorText}`);
                throw new Error(`API Error ${response.status}: ${errorText}`);
            }

            // NovelAI API 返回的是二进制图片数据
            const blob = await response.blob();
            // 将blob转换为base64数据URL，以便保存到聊天记录
            ...
            addLog('NOVELAI_API', `图片生成成功，大小: ${blob.size} bytes`);
            return base64;
        } catch (error) {
            addLog('ERROR', `NovelAI API调用失败: ${error.message}`);
            throw error;
        }
    }

    /**
     * 使用 NovelAI API 生成图片并更新UI
     * @param {Object} state - 当前状态对象
     */
    async function handleNovelAIGeneration(state) {
        if (state.$wrap.data('generating')) return;
        state.$wrap.data('generating', true);

        const config = settings.novelaiConfig;
        if (!config.enabled || !config.apiKey) {
            toastr.error('❌ 请先配置并启用NovelAI API');
            state.$wrap.data('generating', false);
            return;
        }
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                if (attempt > 1) {
                    ...
                    addLog('NOVELAI_GENERATION', `第${attempt}次重试NovelAI生图...`);
                }

                // 根据设置决定是否启用超时
                const genPromise = callNovelAIApi(finalPrompt, config);
                const resultUrl = settings.timeoutEnabled
                    ? await withTimeout(genPromise, settings.timeoutSeconds * 1000)
                    : await genPromise;

                if (resultUrl) {
                    state.el.msg.text('✅ NovelAI成功');
                    const uniqueImages = [...new Set([...state.images, resultUrl])]; // resultUrl已经是base64格式
                    await updateChatData(state.mesId, state.blockIdx, state.prompt, uniqueImages, false, false);
                    setTimeout(() => {
                        const $newWrap = $(`.mes[mesid="${state.mesId}"] .sd-ui-wrap[data-block-idx="${state.blockIdx}"]`);
                        if ($newWrap.length) updateWrapperView($newWrap, uniqueImages, uniqueImages.length - 1);
                    ...
                    lastError = new Error('无结果');
                    if (attempt < MAX_RETRIES) {
                        addLog('NOVELAI_GENERATION', `第${attempt}次尝试无结果，${RETRY_DELAY_MS}ms后重试...`);
                        await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
                    }
                }
            } catch (err) {
                ...
                if (attempt < MAX_RETRIES) {
                    addLog('NOVELAI_GENERATION', `第${attempt}次尝试失败: ${err.message}，${RETRY_DELAY_MS}ms后重试...`);
                    await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
                }
            }
        }
        ...
        if (lastError) {
            ...
            addLog('NOVELAI_GENERATION', `NovelAI生图失败（已重试${MAX_RETRIES}次）: ${lastError.message}`);
        }
