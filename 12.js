...
highly detailed, masterpiece, best quality
    ...
    const DEFAULT_SETTINGS = {
        ...
        // NovelAI API 配置
        novelaiEnabled: false,             // NovelAI API 开关
        novelaiApiKey: '',                 // NovelAI API 密钥
        novelaiModel: 'nai-diffusion-3',   // NovelAI 模型名称（可手动输入）
        novelaiParameters: {               // NovelAI 生图参数
            model: 'nai-diffusion-3',      // 模型名称
            sampler: 'k_euler',
            ...
            sm: false,
            sm_dyn: false,
            decrisper: false,              // dynamic_thresholding
            cfg_rescale: 0,
            variety_boost: false,          // 用于v4.5模型
            width: 1024,                   // 图片宽度
            height: 1024,                  // 图片高度
            n_samples: 1,                  // 生成样本数
    ...
    async function handleGeneration(state) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                ...
                // 根据设置决定使用哪种API
                if (settings.novelaiEnabled && settings.novelaiApiKey) {
                    // 使用 NovelAI API
                    ...
                    const novelaiParams = {
                        ...
                        // 使用当前设置的模型，如果设置了自定义模型则使用自定义模型
                        model: settings.novelaiModel,
                        width: settings.novelaiParameters.width,
                        height: settings.novelaiParameters.height,
                        sampler: settings.novelaiParameters.sampler,
                        scheduler: settings.novelaiParameters.scheduler,
                        steps: settings.novelaiParameters.steps,
                        scale: settings.novelaiParameters.scale,
                        seed: settings.novelaiParameters.seed,
                        noise: settings.novelaiParameters.noise,
                        strength: settings.novelaiParameters.strength,
                        ucPreset: settings.novelaiParameters.ucPreset,
                        qualityToggle: settings.novelaiParameters.qualityToggle,
                        autoSmea: settings.novelaiParameters.autoSmea,
                        sm: settings.novelaiParameters.sm,
                        sm_dyn: settings.novelaiParameters.sm_dyn,
                        decrisper: settings.novelaiParameters.decrisper,
                        cfg_rescale: settings.novelaiParameters.cfg_rescale,
                        variety_boost: settings.novelaiParameters.variety_boost,
                        n_samples: settings.novelaiParameters.n_samples,
                        controlnet_strength: settings.novelaiParameters.controlnet_strength,
                        legacy: settings.novelaiParameters.legacy,
                        add_original_image: settings.novelaiParameters.add_original_image,
                        image_format: settings.novelaiParameters.image_format
                    };

                    // 根据设置决定是否启用超时
                    const novelaiPromise = callNovelAIApi(novelaiParams, settings.novelaiApiKey);
                    const result = settings.timeoutEnabled
                        ? await withTimeout(novelaiPromise, settings.timeoutSeconds * 1000)
                        : await novelaiPromise;

                    if (result) {
                        newUrls = [result]; // NovelAI 返回的是 base64 数据URL
                    }
                } else {
                    // 使用原有方式
                    const finalPrompt = `${settings.globalPrefix ? settings.globalPrefix + ', ' : ''}${state.prompt}${settings.globalSuffix ? ', ' + settings.globalSuffix : ''}`.replace(/,\s*,/g, ',').trim();
                    ...
                    // 保持原始URL格式，仅清理尾部空白
                    newUrls = newUrls.map(url => url.trim());
                }
    ...
    // NovelAI API 调用函数
    async function callNovelAIApi(params, apiKey) {
        try {
            // 确定模型名称
            const modelName = params.model || settings.novelaiModel;

            // 如果seed为-1，生成随机种子
            const seed = (params.seed !== undefined && params.seed !== -1) ? params.seed :
                         (settings.novelaiParameters.seed !== -1) ? settings.novelaiParameters.seed :
            ...
            if (isV3) {
                ...
                payload = {
                    ...
                    model: modelName,
                    parameters: {
                        width: params.width || settings.novelaiParameters.width,
                        height: params.height || settings.novelaiParameters.height,
                        scale: params.scale || settings.novelaiParameters.scale,
                        seed: seed,
                        sampler: params.sampler || settings.novelaiParameters.sampler,
                        noise_schedule: params.scheduler || settings.novelaiParameters.scheduler,
                        steps: params.steps || settings.novelaiParameters.steps,
                        n_samples: params.n_samples || settings.novelaiParameters.n_samples,
                        ucPreset: params.ucPreset || settings.novelaiParameters.ucPreset,
                        qualityToggle: params.qualityToggle !== undefined ? params.qualityToggle : settings.novelaiParameters.qualityToggle,
                        sm: params.sm !== undefined ? params.sm : settings.novelaiParameters.sm,
                        sm_dyn: params.sm_dyn !== undefined ? params.sm_dyn : settings.novelaiParameters.sm_dyn,
                        dynamic_thresholding: params.decrisper !== undefined ? params.decrisper : settings.novelaiParameters.decrisper,
                        controlnet_strength: params.controlnet_strength || settings.novelaiParameters.controlnet_strength,
                        legacy: params.legacy !== undefined ? params.legacy : settings.novelaiParameters.legacy,
                        add_original_image: params.add_original_image !== undefined ? params.add_original_image : settings.novelaiParameters.add_original_image,
                        cfg_rescale: params.cfg_rescale || settings.novelaiParameters.cfg_rescale,
                        uncond_scale: params.uncond_scale || settings.novelaiParameters.uncond_scale,
                        image_format: params.image_format || settings.novelaiParameters.image_format,
                        strength: params.strength || settings.novelaiParameters.strength,
                        noise: params.noise || settings.novelaiParameters.noise
                    }
                };
                ...
                if (params.negative_prompt) {
                    ...
                    negativePrompt = settings.globalNegative || '';
                }
                ...
                if (params.variety_boost || settings.novelaiParameters.variety_boost) {
                    const width = params.width || settings.novelaiParameters.width;
                    const height = params.height || settings.novelaiParameters.height;
                ...
                payload = {
                    ...
                    parameters: {
                        ...
                        ucPreset: params.ucPreset || settings.novelaiParameters.ucPreset,
                        qualityToggle: params.qualityToggle !== undefined ? params.qualityToggle : settings.novelaiParameters.qualityToggle,
                        autoSmea: params.autoSmea !== undefined ? params.autoSmea : settings.novelaiParameters.autoSmea,
                        cfg_rescale: params.cfg_rescale || settings.novelaiParameters.cfg_rescale,
                        ...
                        negative_prompt: params.negative_prompt || settings.globalNegative || ''
                    }
                };
            } else {
                // 默认使用V3参数结构
                ...
                // 添加负面提示词
                payload.parameters.negative_prompt = params.negative_prompt || settings.globalNegative || '';
            }

            // 使用原生fetch，避免使用safeFetch以防止潜在的递归调用
            ...
            // NovelAI API 返回的是二进制数据，需要转换为base64
            ...
            // 避免在错误处理中再次调用可能导致递归的函数
            console.error('NOVELAI_ERROR', `NovelAI API调用失败: ${error.message}`);
            // 只记录到控制台，避免可能的递归调用
            throw error;
        }
    }
    ...
    function openSettingsPopup() {
        ...
        const html = `
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div class="sd-tab-nav">
                    ...
                    <div class="sd-tab-btn" data-tab="novelai-api">NovelAI</div>
                ...
                <!-- Tab 4: NovelAI API 配置 -->
                <div id="sd-tab-novelai-api" class="sd-tab-content">
                    <h4 style="margin-top:0; margin-bottom:15px;">NovelAI API 配置</h4>

                    <div style="margin-bottom: 12px;">
                        <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                            <input type="checkbox" id="sd-novelai-en" ${settings.novelaiEnabled ? 'checked' : ''}>
                            <span style="font-weight: bold;">启用 NovelAI API</span>
                        </label>
                        <small style="color: #888; display: block; margin-left: 24px; margin-top: 4px;">
                            开启后将使用 NovelAI API 进行生图，关闭则使用原有方式
                        </small>
                    </div>
                    ...
                    <div class="sd-api-row">
                        <label>模型名称</label>
                        <div style="display: flex; gap: 8px; align-items: center;">
                            <select id="sd-novelai-model" class="text_pole" style="flex: 1;">
                                <option value="${settings.novelaiModel}" id="current-model-option">${settings.novelaiModel}</option>
                                <option value="__CUSTOM__">自定义模型...</option>
                            </select>
                            <input type="text" id="sd-novelai-model-custom" class="text_pole" placeholder="输入自定义模型" style="flex: 1; display: none;" value="${settings.novelaiModel}">
                            <button id="sd-fetch-novelai-models" class="sd-btn-secondary" style="flex:0 0 80px;">获取</button>
                        </div>
                    </div>

                    <hr style="border: none; border-top: 1px solid rgba(255,255,255,0.1); margin: 20px 0;">

                    <h4 style="margin-bottom:15px;">生图参数</h4>

                    <div class="sd-api-row">
                        <label>采样器</label>
                        <select id="sd-novelai-sampler" class="text_pole">
                            <option value="k_euler" ${settings.novelaiParameters.sampler === 'k_euler' ? 'selected' : ''}>Euler (k_euler)</option>
                            <option value="k_euler_ancestral" ${settings.novelaiParameters.sampler === 'k_euler_ancestral' ? 'selected' : ''}>Euler ancestral (k_euler_ancestral)</option>
                            <option value="k_dpmpp_2s_ancestral" ${settings.novelaiParameters.sampler === 'k_dpmpp_2s_ancestral' ? 'selected' : ''}>DPM++ 2S ancestral (k_dpmpp_2s_ancestral)</option>
                            <option value="k_dpmpp_2m" ${settings.novelaiParameters.sampler === 'k_dpmpp_2m' ? 'selected' : ''}>DPM++ 2M (k_dpmpp_2m)</option>
                            <option value="k_dpmpp_sde" ${settings.novelaiParameters.sampler === 'k_dpmpp_sde' ? 'selected' : ''}>DPM++ SDE (k_dpmpp_sde)</option>
                            <option value="ddim" ${settings.novelaiParameters.sampler === 'ddim' ? 'selected' : ''}>DDIM</option>
                        </select>
                    </div>

                    <div class="sd-api-row">
                        <label>步数</label>
                        <input type="number" id="sd-novelai-steps" class="text_pole" value="${settings.novelaiParameters.steps}" min="1" max="50" step="1">
                    </div>

                    <div class="sd-api-row">
                        <label>CFG Scale</label>
                        <input type="number" id="sd-novelai-scale" class="text_pole" value="${settings.novelaiParameters.scale}" min="1" max="20" step="0.5">
                    </div>

                    <div class="sd-api-row">
                        <label>Seed</label>
                        <input type="number" id="sd-novelai-seed" class="text_pole" value="${settings.novelaiParameters.seed}" min="-1" max="999999999" step="1">
                    ...
                    <div class="sd-api-row">
                        <label>噪声</label>
                        <input type="number" id="sd-novelai-noise" class="text_pole" value="${settings.novelaiParameters.noise}" min="0" max="1" step="0.01">
                    </div>

                    <div class="sd-api-row">
                        <label>强度</label>
                        <input type="number" id="sd-novelai-strength" class="text_pole" value="${settings.novelaiParameters.strength}" min="0" max="1" step="0.01">
                    </div>

                    <div class="sd-api-row">
                        <label>UC Preset</label>
                        <select id="sd-novelai-uc-preset" class="text_pole">
                            <option value="0" ${settings.novelaiParameters.ucPreset === 0 ? 'selected' : ''}>0 (None)</option>
                            <option value="1" ${settings.novelaiParameters.ucPreset === 1 ? 'selected' : ''}>1 (Light)</option>
                            <option value="2" ${settings.novelaiParameters.ucPreset === 2 ? 'selected' : ''}>2 (Heavy)</option>
                        </select>
                    </div>

                    <div class="sd-api-row">
                        <label>Quality Toggle</label>
                        <input type="checkbox" id="sd-novelai-quality-toggle" ${settings.novelaiParameters.qualityToggle ? 'checked' : ''}>
                    </div>

                    <div class="sd-api-row">
                        <label>SM</label>
                        <input type="checkbox" id="sd-novelai-sm" ${settings.novelaiParameters.sm ? 'checked' : ''}>
                    </div>

                    <div class="sd-api-row">
                        <label>SM Dynamic</label>
                        <input type="checkbox" id="sd-novelai-sm-dyn" ${settings.novelaiParameters.sm_dyn ? 'checked' : ''}>
                    </div>

                    <div class="sd-api-row">
                        <label>动态阈值</label>
                        <input type="checkbox" id="sd-novelai-dyn-threshold" ${settings.novelaiParameters.dynamicThresholding ? 'checked' : ''}>
                    </div>

                    <div class="sd-api-row">
                        <label>CFG Rescale</label>
                        <input type="number" id="sd-novelai-cfg-rescale" class="text_pole" value="${settings.novelaiParameters.cfg_rescale || 0}" min="0" max="1" step="0.01">
                    </div>

                    <div class="sd-api-row">
                        <label>宽度</label>
                        <input type="number" id="sd-novelai-width" class="text_pole" value="${settings.novelaiParameters.width}" min="64" max="2048" step="64">
                    </div>

                    <div class="sd-api-row">
                        <label>高度</label>
                        <input type="number" id="sd-novelai-height" class="text_pole" value="${settings.novelaiParameters.height}" min="64" max="2048" step="64">
                    </div>

                    <button id="sd-test-novelai-api" class="sd-btn-secondary" style="width:100%; margin-top:10px;">🧪 测试 NovelAI API 连接</button>
                </div>
        ...
        setTimeout(() => {
            ...
            // 初始化模型选择界面
            const initialModel = $('#sd-novelai-model').val();
            if (initialModel === '__CUSTOM__' || !$('#sd-novelai-model option[value="' + settings.novelaiModel + '"]').length) {
                // 如果当前值是自定义选项或当前模型不在选项中，则添加当前模型选项并选择它
                if (!$('#sd-novelai-model option[value="' + settings.novelaiModel + '"]').length) {
                    $('#sd-novelai-model').prepend(`<option value="${settings.novelaiModel}" selected>${settings.novelaiModel}</option>`);
                }
                $('#sd-novelai-model-custom').show().val(settings.novelaiModel);
                $('#sd-novelai-model').val('__CUSTOM__');
            } else {
                $('#sd-novelai-model-custom').hide();
            }

            // 模型选择变化事件处理
            $('#sd-novelai-model').on('change', function () {
                const selectedValue = $(this).val();
                const $customInput = $('#sd-novelai-model-custom');

                if (selectedValue === '__CUSTOM__') {
                    // 选择自定义模型，显示输入框
                    $customInput.show();
                    $customInput.val(settings.novelaiModel); // 设置为当前模型值
                } else {
                    // 选择预设模型，隐藏输入框
                    $customInput.hide();
                }
            ...
            // 获取 NovelAI 模型列表
            $('#sd-fetch-novelai-models').on('click', async function () {
                const $btn = $(this);
                ...
                try {
                    ...
                    if (models && Array.isArray(models)) {
                        ...
                        models.forEach(model => {
                            const modelName = typeof model === 'string' ? model : (model.id || model.name || model);
                            $select.append(`<option value="${modelName}">${modelName}</option>`);
                        });

                        // 添加自定义选项
                        $select.append('<option value="__CUSTOM__">自定义模型...</option>');

                        if (models.length === 0) {
                            $select.append(`<option value="${settings.novelaiModel}">${settings.novelaiModel}</option>`);
                            $select.append('<option value="__CUSTOM__">自定义模型...</option>');
                            toastr.warning('未获取到模型列表');
                        } else {
                            toastr.success(`✅ 成功获取 ${models.length} 个模型`);
                        ...
                        $select.append(`<option value="${settings.novelaiModel}">${settings.novelaiModel}</option>`);
                        $select.append('<option value="__CUSTOM__">自定义模型...</option>');
                        toastr.warning('未获取到有效的模型列表');
                    }

                    // 恢复之前的选择，如果是自定义值则选择自定义选项
                    if (currentValue !== '__CUSTOM__' && currentValue !== settings.novelaiModel) {
                        // 如果之前选的是一个获取到的模型，尝试重新选择它
                        if ($(`#sd-novelai-model option[value="${currentValue}"]`).length > 0) {
                            $select.val(currentValue);
                        } else {
                            // 否则选择自定义选项
                            $select.val('__CUSTOM__');
                            $('#sd-novelai-model-custom').show().val(currentValue);
                        }
                    }
                } catch (e) {
                    ...
                    if (currentValue === '__CUSTOM__') {
                        $select.val('__CUSTOM__');
                        $('#sd-novelai-model-custom').show();
                    } else {
                        $select.val(settings.novelaiModel);
                    }
                } finally {
            ...
            // 测试 NovelAI API
            $('#sd-test-novelai-api').on('click', async function () {
                const $btn = $(this);
                const apiKey = $('#sd-novelai-key').val();
                const $select = $('#sd-novelai-model');
                ...
                if (selectedModel === '__CUSTOM__') {
                    model = $('#sd-novelai-model-custom').val().trim();
                    if (!model) {
                        toastr.warning('请输入自定义模型名称');
                        return;
                    }
                } else {
                    model = selectedModel;
                ...
                try {
                    // 创建临时的测试参数
                    const testParams = {
                        input: "1girl, long hair, blue dress",
                        model: model,
                        sampler: $('#sd-novelai-sampler').val(),
                        scheduler: 'karras', // 默认调度器
                        steps: parseInt($('#sd-novelai-steps').val()) || 28,
                        scale: parseFloat($('#sd-novelai-scale').val()) || 5,
                        seed: parseInt($('#sd-novelai-seed').val()) || -1,
                        noise: parseFloat($('#sd-novelai-noise').val()) || 0.2,
                        strength: parseFloat($('#sd-novelai-strength').val()) || 0.7,
                        ucPreset: parseInt($('#sd-novelai-uc-preset').val()) || 0,
                        qualityToggle: $('#sd-novelai-quality-toggle').is(':checked'),
                        autoSmea: false, // 默认值
                        sm: $('#sd-novelai-sm').is(':checked'),
                        sm_dyn: $('#sd-novelai-sm-dyn').is(':checked'),
                        decrisper: $('#sd-novelai-dyn-threshold').is(':checked'), // 重命名为decrisper
                        cfg_rescale: 0, // 默认值
                        variety_boost: false, // 默认值
                        n_samples: 1, // 默认值
                        controlnet_strength: 1.0, // 默认值
                        legacy: false, // 默认值
                        add_original_image: false, // 默认值
                        image_format: 'png', // 默认值
                        width: parseInt($('#sd-novelai-width').val()) || 1024,
                        height: parseInt($('#sd-novelai-height').val()) || 1024
                    };

                    // 尝试调用 NovelAI API
                    const response = await callNovelAIApi(testParams, apiKey);

                    if (response && response.length > 0) {
                        toastr.success('✅ NovelAI API连接测试成功！');
                    } else {
                        throw new Error('API返回无效结果');
                    }
                } catch (e) {
                    toastr.error(`❌ NovelAI API测试失败: ${e.message}`);
                } finally {
                    $btn.prop('disabled', false).text('🧪 测试 NovelAI API 连接');
                }
            });
            ...
            $('#sd-save').on('click', () => {
                ...
                // 处理模型选择：如果是自定义模型，使用自定义输入框的值
                const selectedModel = $('#sd-novelai-model').val();
                if (selectedModel === '__CUSTOM__') {
                    settings.novelaiModel = $('#sd-novelai-model-custom').val().trim() || settings.novelaiModel;
                } else {
                    settings.novelaiModel = selectedModel;
                }

                settings.novelaiParameters = {
                    model: settings.novelaiModel, // 保留模型名称
                    sampler: $('#sd-novelai-sampler').val(),
                    scheduler: 'karras', // 固定调度器
                    steps: parseInt($('#sd-novelai-steps').val()) || 28,
                    scale: parseFloat($('#sd-novelai-scale').val()) || 5,
                    seed: parseInt($('#sd-novelai-seed').val()) || -1,
                    noise: parseFloat($('#sd-novelai-noise').val()) || 0.2,
                    ...
                    sm_dyn: $('#sd-novelai-sm-dyn').is(':checked'),
                    decrisper: $('#sd-novelai-dyn-threshold').is(':checked'), // 重命名为decrisper
                    cfg_rescale: parseFloat($('#sd-novelai-cfg-rescale').val()) || 0,
                    variety_boost: false, // 默认值
                    width: parseInt($('#sd-novelai-width').val()) || 1024,
                    height: parseInt($('#sd-novelai-height').val()) || 1024,
                    n_samples: 1, // 默认值
