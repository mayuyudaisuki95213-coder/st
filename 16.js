            ...
            for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
                try {
                    ...
                    if (result) {
                        ...
                        // 如果是 base64 图片，不存储到文本中，只更新 UI
                        let uniqueImages;
                        if (result.startsWith('data:image/')) {
                            // base64 图片，不存储到文本，只添加到当前视图
            ...
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div id="sd-tab-novelai" class="sd-tab-content">
                    ...
                    <div style="margin-top: 20px; padding: 15px; background: linear-gradient(145deg, #252530, #1e1e24); border-radius: 8px; box-shadow: 3px 3px 6px var(--nm-shadow-dark), -2px -2px 5px var(--nm-shadow-light);">
                        ...
                        <div class="sd-api-row">
                            <label>宽度</label>
                            <input type="number" id="sd-novelai-width" class="text_pole" value="${settings.novelaiParams.width}" min="64" max="2048" step="64">
                        </div>

                        <div class="sd-api-row">
                            <label>高度</label>
                            <input type="number" id="sd-novelai-height" class="text_pole" value="${settings.novelaiParams.height}" min="64" max="2048" step="64">
                        </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-save').on('click', () => {
                ...
                settings.novelaiParams = {
                    width: parseInt($('#sd-novelai-width').val()) || 1216,
                    height: parseInt($('#sd-novelai-height').val()) || 832,
            ...
            $('#sd-test-novelai').on('click', async function () {
                ...
                try {
                    ...
                    settings.novelaiParams = {
                        width: parseInt($('#sd-novelai-width').val()) || 640,
                        height: parseInt($('#sd-novelai-height').val()) || 480,
                        scale: parseFloat($('#sd-novelai-scale').val()) || 5,
                        steps: parseInt($('#sd-novelai-steps').val()) || 20,
                        seed: -1,
                        sampler: $('#sd-novelai-sampler').val(),
                        scheduler: $('#sd-novelai-scheduler').val(),
                        qualityToggle: $('#sd-novelai-quality-toggle').is(':checked'),
