...
highly detailed, masterpiece, best quality
    ...
    const DEFAULT_SETTINGS = {
        ...
        // NovelAI API 配置
        novelaiEnabled: false,             // NovelAI直连开关
        novelaiApiKey: '',                 // NovelAI API密钥
        novelaiModel: 'nai-diffusion-3',   // NovelAI模型
        novelaiSampler: 'k_euler',         // NovelAI采样器
        novelaiSteps: 28,                  // 生成步数
        novelaiCfgScale: 7,                // 提示词引导系数
        novelaiNoise: 0.2,                // 噪声
        novelaiStrength: 0.7,             // 强度
        novelaiWidth: 1024,               // 图像宽度
        novelaiHeight: 1024,              // 图像高度
    ...
    async function handleGeneration(state) {
        ...
        if (settings.novelaiEnabled) {
            ...
            // 使用原有酒馆生图功能
            await handleTavernGeneration(state);
        }
    }

    async function handleTavernGeneration(state) {
    ...
    async function handleNovelAIGeneration(state) {
        ...
        for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
            try {
                if (attempt > 1) {
                    state.el.msg.text(`⏳ 重试中 (${attempt}/${MAX_RETRIES})...`);
                    addLog('NOVELAI', `第${attempt}次重试 NovelAI 生图...`);
                }

                // 根据设置决定是否启用超时
                const novelaiPromise = callNovelAIApi(finalPrompt, settings);
                const imageUrl = settings.timeoutEnabled
                ...
                if (imageUrl) {
                    ...
                    lastError = new Error('无结果');
                    if (attempt < MAX_RETRIES) {
                        addLog('NOVELAI', `第${attempt}次尝试无结果，${RETRY_DELAY_MS}ms后重试...`);
                        await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
                    }
                }
            } catch (err) {
                ...
                if (attempt < MAX_RETRIES) {
                    addLog('NOVELAI', `第${attempt}次尝试失败: ${err.message}，${RETRY_DELAY_MS}ms后重试...`);
                    await new Promise(resolve => setTimeout(resolve, RETRY_DELAY_MS));
                }
            }
        }
        ...
        if (lastError) {
            ...
            addLog('NOVELAI', `NovelAI 生图失败（已重试${MAX_RETRIES}次）: ${lastError.message}`);
        }
    ...
    function openSettingsPopup() {
        ...
        const html = `
            <div class="sd-settings-popup" style="display: flex; flex-direction: column; max-height: 78vh;">
                ...
                <div id="sd-tab-basic" class="sd-tab-content active">
                    ...
                    <!-- NovelAI API 配置区 -->
                    <div style="margin-top: 20px; padding: 12px; background: linear-gradient(145deg, #252530, #1e1e24); border-radius: 8px; box-shadow: 3px 3px 6px var(--nm-shadow-dark), -2px -2px 5px var(--nm-shadow-light);">
                        <div style="display: flex; align-items: center; gap: 8px; margin-bottom: 10px;">
                            <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                <input type="checkbox" id="sd-novelai-en" ${settings.novelaiEnabled ? 'checked' : ''}>
                                <span style="font-weight: 600; color: #ff6b6b;">🚀 NovelAI 直连模式</span>
                            </label>
                            <span class="sd-toggle-arrow collapsed" data-target="sd-sub-novelai">▾</span>
                        </div>
                        <small style="color: #888; display: block; margin-left: 24px; margin-top: 4px;">
                            开启后将绕过酒馆内置生图功能，直接调用 NovelAI API 生成图片
                        </small>
                        <div id="sd-sub-novelai" class="sd-sub-settings collapsed" style="display: flex; flex-wrap: wrap; gap: 12px; align-items: center; margin-top: 10px;">
                            ...
                            <div style="flex: 1; min-width: 150px;">
                                <label style="display:block; margin-bottom:5px; font-size: 12px;">模型</label>
                                <select id="sd-novelai-model" class="text_pole">
                                    ...
                                    <option value="furry" ${settings.novelaiModel === 'furry' ? 'selected' : ''}>furry</option>
                                </select>
                            </div>
                            <div style="flex: 1; min-width: 150px;">
                                <label style="display:block; margin-bottom:5px; font-size: 12px;">采样器</label>
                                <select id="sd-novelai-sampler" class="text_pole">
                                    <option value="k_euler" ${settings.novelaiSampler === 'k_euler' ? 'selected' : ''}>Euler</option>
                                    <option value="k_euler_ancestral" ${settings.novelaiSampler === 'k_euler_ancestral' ? 'selected' : ''}>Euler a</option>
                                    <option value="k_dpmpp_2m" ${settings.novelaiSampler === 'k_dpmpp_2m' ? 'selected' : ''}>DPM++ 2M</option>
                                    <option value="k_dpmpp_2s_ancestral" ${settings.novelaiSampler === 'k_dpmpp_2s_ancestral' ? 'selected' : ''}>DPM++ 2S a</option>
                                </select>
                            </div>
                            <div style="flex: 1; min-width: 120px;">
                                <label style="display:block; margin-bottom:5px; font-size: 12px;">步数</label>
                                <input type="number" id="sd-novelai-steps" class="text_pole" value="${settings.novelaiSteps}" min="1" max="50" style="width:100%;">
                            </div>
                            <div style="flex: 1; min-width: 120px;">
                                <label style="display:block; margin-bottom:5px; font-size: 12px;">CFG Scale</label>
                                <input type="number" id="sd-novelai-cfg" class="text_pole" value="${settings.novelaiCfgScale}" min="1" max="20" step="0.5" style="width:100%;">
                            </div>
                            <div style="flex: 1; min-width: 120px;">
                                <label style="display:block; margin-bottom:5px; font-size: 12px;">宽度</label>
                                <input type="number" id="sd-novelai-width" class="text_pole" value="${settings.novelaiWidth}" min="64" max="2048" step="64" style="width:100%;">
                            </div>
                            <div style="flex: 1; min-width: 120px;">
                                <label style="display:block; margin-bottom:5px; font-size: 12px;">高度</label>
                                <input type="number" id="sd-novelai-height" class="text_pole" value="${settings.novelaiHeight}" min="64" max="2048" step="64" style="width:100%;">
                            </div>
                            <div style="flex: 1; min-width: 100%;">
                                <label style="display:block; margin-bottom:5px; font-size: 12px;">负面提示词</label>
                                <textarea id="sd-novelai-negative" class="text_pole" rows="3" style="width:100%;">${settings.novelaiNegativePrompt}</textarea>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        setTimeout(() => {
            ...
            $('#sd-save').on('click', () => {
                ...
                settings.novelaiSampler = $('#sd-novelai-sampler').val();
                settings.novelaiSteps = parseInt($('#sd-novelai-steps').val()) || 28;
                settings.novelaiCfgScale = parseFloat($('#sd-novelai-cfg').val()) || 7;
                settings.novelaiWidth = parseInt($('#sd-novelai-width').val()) || 1024;
                settings.novelaiHeight = parseInt($('#sd-novelai-height').val()) || 1024;
                settings.novelaiNegativePrompt = $('#sd-novelai-negative').val();
     ...
     * @param {Object} config - NovelAI 配置对象
     * @returns {Promise<string>} - 返回图片URL或base64数据
     */
    async function callNovelAIApi(prompt, config) {
        if (!config.novelaiApiKey) {
        ...
        const requestBody = {
            ...
            action: "generate",
            parameters: {
                width: config.novelaiWidth || 1024,
                height: config.novelaiHeight || 1024,
                scale: config.novelaiCfgScale || 7,
                sampler: config.novelaiSampler || 'k_euler',
                steps: config.novelaiSteps || 28,
                seed: -1,
                n_samples: 1,
                uc: config.novelaiNegativePrompt || 'lowres, bad anatomy, bad hands, text, error, missing fingers, extra digit, fewer digits, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry',
                noise: config.novelaiNoise || 0.2,
                strength: config.novelaiStrength || 0.7
            }
        };

        addLog('NOVELAI', `请求 NovelAI API: ${url}`);
        addLog('NOVELAI', `模型: ${requestBody.model}, 尺寸: ${requestBody.parameters.width}x${requestBody.parameters.height}`);
        ...
        try {
            const res = await safeFetch(url, {
                ...
                headers: {
                    "Content-Type": "application/json",
                    "Authorization": `Bearer ${config.novelaiApiKey}`,
            ...
            if (!res.ok) {
                const errorText = await res.text();
                addLog('ERROR', `NovelAI API响应: ${res.status} - ${errorText}`);
                throw new Error(`NovelAI API Error ${res.status}: ${errorText}`);
            }

            // NovelAI API 返回的是 base64 编码的图像数据
            const data = await res.json();

            if (data && data.images && data.images.length > 0) {
                // NovelAI 返回的是 base64 编码的数据，需要转换为 URL
                const base64Data = data.images[0];
                const imageUrl = `data:image/png;base64,${base64Data}`;

                addLog('NOVELAI', `图片生成成功，大小: ${imageUrl.length} 字符`);
                return imageUrl;
            } else {
                throw new Error("NovelAI API 返回数据格式错误");
            }
        } catch (error) {
